...
export function loginAdmin(username: string, password: string, userAgent?: string | null) {
  ...
  const admin = db.prepare('SELECT id, username, display_name, password_hash, role, active FROM admin_users WHERE username = ? COLLATE NOCASE').get(username) as any;
  ...
  return { token: rawToken, expires, admin: { id: admin.id, username: admin.username, displayName: admin.display_name, role: admin.role } };
}
...
export function getAdminFromRequest(request: NextRequest) {
  ...
  const db = getDatabase();
  const row = db.prepare(`SELECT a.id, a.username, a.display_name, a.role, s.id AS session_id
