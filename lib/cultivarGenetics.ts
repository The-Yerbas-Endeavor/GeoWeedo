...
function ensureSchema() {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS cannabis_cultivar_sources (
      id TEXT PRIMARY KEY,
    ...
    CREATE TABLE IF NOT EXISTS cannabis_cultivar_aliases (
      ...
      FOREIGN KEY(source_id) REFERENCES cannabis_cultivar_sources(id) ON DELETE SET NULL,
      UNIQUE(cultivar_id, normalized_alias)
    );

    CREATE TABLE IF NOT EXISTS cannabis_cultivar_lineage_claims (
      id TEXT PRIMARY KEY,
      ...
      created_at TEXT NOT NULL,
      updated_at TEXT NOT NULL,
      FOREIGN KEY(child_cultivar_id) REFERENCES cannabis_cultivars(id) ON DELETE CASCADE,
      FOREIGN KEY(parent_cultivar_id) REFERENCES cannabis_cultivars(id) ON DELETE CASCADE,
      FOREIGN KEY(source_id) REFERENCES cannabis_cultivar_sources(id) ON DELETE SET NULL
    );

    CREATE TABLE IF NOT EXISTS cannabis_product_cultivars (
      id TEXT PRIMARY KEY,
    ...
    CREATE TABLE IF NOT EXISTS cannabis_cultivar_genetic_relationships (
      id TEXT PRIMARY KEY,
      ...
      created_at TEXT NOT NULL,
      updated_at TEXT NOT NULL,
      FOREIGN KEY(cultivar_a_id) REFERENCES cannabis_cultivars(id) ON DELETE CASCADE,
      FOREIGN KEY(cultivar_b_id) REFERENCES cannabis_cultivars(id) ON DELETE CASCADE,
      FOREIGN KEY(source_id) REFERENCES cannabis_cultivar_sources(id) ON DELETE CASCADE
    );

    CREATE INDEX IF NOT EXISTS cannabis_cultivars_name_idx ON cannabis_cultivars(normalized_name);
    CREATE INDEX IF NOT EXISTS cannabis_cultivar_aliases_name_idx ON cannabis_cultivar_aliases(normalized_alias);
    CREATE INDEX IF NOT EXISTS cannabis_cultivar_lineage_child_idx ON cannabis_cultivar_lineage_claims(child_cultivar_id, confidence DESC);
    CREATE INDEX IF NOT EXISTS cannabis_cultivar_lineage_parent_idx ON cannabis_cultivar_lineage_claims(parent_cultivar_id, confidence DESC);
    CREATE INDEX IF NOT EXISTS cannabis_product_cultivars_product_idx ON cannabis_product_cultivars(product_id, confidence DESC);
    CREATE INDEX IF NOT EXISTS cannabis_product_cultivars_cultivar_idx ON cannabis_product_cultivars(cultivar_id, confidence DESC);
    CREATE INDEX IF NOT EXISTS cannabis_cultivar_genetic_a_idx ON cannabis_cultivar_genetic_relationships(cultivar_a_id);
    CREATE INDEX IF NOT EXISTS cannabis_cultivar_genetic_b_idx ON cannabis_cultivar_genetic_relationships(cultivar_b_id);
  `);
  return db;
}
...
export function createOrEnrichCultivar(input: {
  ...
  const existing = db.prepare('SELECT * FROM cannabis_cultivars WHERE normalized_name=? LIMIT 1').get(normalized) as any;
  if (existing) {
    db.prepare(`UPDATE cannabis_cultivars SET
    ...
    return db.prepare('SELECT * FROM cannabis_cultivars WHERE id=?').get(existing.id) as any;
  }
  const id = `cultivar-${randomUUID()}`;
  db.prepare(`INSERT INTO cannabis_cultivars
  ...
  return db.prepare('SELECT * FROM cannabis_cultivars WHERE id=?').get(id) as any;
}
...
export function addCultivarSource(input: {
  ...
  const existing = db.prepare(`SELECT * FROM cannabis_cultivar_sources
  ...
  if (existing) {
    db.prepare(`UPDATE cannabis_cultivar_sources SET
    ...
    return db.prepare('SELECT * FROM cannabis_cultivar_sources WHERE id=?').get(existing.id) as any;
  }
  const id = `cultsrc-${randomUUID()}`;
  db.prepare(`INSERT INTO cannabis_cultivar_sources
  ...
  return db.prepare('SELECT * FROM cannabis_cultivar_sources WHERE id=?').get(id) as any;
}
...
export function addCultivarAlias(input: { cultivarId: string; alias: string; sourceId?: string | null; verified?: boolean }) {
  ...
  const cultivar = db.prepare('SELECT id FROM cannabis_cultivars WHERE id=?').get(input.cultivarId);
  if (!cultivar) throw new Error('Cultivar was not found.');
  const existing = db.prepare('SELECT id FROM cannabis_cultivar_aliases WHERE cultivar_id=? AND normalized_alias=?').get(input.cultivarId, normalized) as any;
  if (existing?.id) {
    if (input.verified) db.prepare('UPDATE cannabis_cultivar_aliases SET verified=1 WHERE id=?').run(existing.id);
    return existing.id as string;
  }
  const id = `cultalias-${randomUUID()}`;
  db.prepare(`INSERT INTO cannabis_cultivar_aliases (id,cultivar_id,alias,normalized_alias,source_id,verified,created_at)
    VALUES (?,?,?,?,?,?,?)`)
...
export function addLineageClaim(input: {
  ...
  const child = db.prepare('SELECT id FROM cannabis_cultivars WHERE id=?').get(input.childCultivarId);
  const parent = db.prepare('SELECT id FROM cannabis_cultivars WHERE id=?').get(input.parentCultivarId);
  ...
  const existing = db.prepare(`SELECT id FROM cannabis_cultivar_lineage_claims
  ...
  const now = new Date().toISOString();
  if (existing?.id) {
    db.prepare(`UPDATE cannabis_cultivar_lineage_claims SET generation=?,confidence=?,status=?,notes=?,updated_at=? WHERE id=?`)
  ...
  db.prepare(`INSERT INTO cannabis_cultivar_lineage_claims
...
export function linkProductToCultivar(input: {
  ...
  const sourceId = clean(input.sourceId);
  const existing = db.prepare(`SELECT id FROM cannabis_product_cultivars
  ...
  const confidence = Math.max(0, Math.min(100, Number(input.confidence ?? 50)));
  if (existing?.id) {
    db.prepare('UPDATE cannabis_product_cultivars SET confidence=?,status=?,notes=?,updated_at=? WHERE id=?')
      .run(confidence, clean(input.status) || 'single_source_claim', clean(input.notes), now, existing.id);
    return existing.id as string;
  }
  const id = `prodcult-${randomUUID()}`;
  db.prepare(`INSERT INTO cannabis_product_cultivars
...
export function addGeneticRelationship(input: {
  ...
  const source = db.prepare('SELECT id FROM cannabis_cultivar_sources WHERE id=?').get(input.sourceId);
  const a = db.prepare('SELECT id FROM cannabis_cultivars WHERE id=?').get(ids[0]);
  const b = db.prepare('SELECT id FROM cannabis_cultivars WHERE id=?').get(ids[1]);
  ...
  const existing = db.prepare(`SELECT id FROM cannabis_cultivar_genetic_relationships
  ...
  if (existing?.id) {
    db.prepare(`UPDATE cannabis_cultivar_genetic_relationships SET similarity_score=?,evidence_json=COALESCE(?,evidence_json),verified=MAX(verified,?),updated_at=? WHERE id=?`)
      .run(Number.isFinite(score as number) ? score : null, evidenceJson, input.verified ? 1 : 0, now, existing.id);
    return existing.id as string;
  }
  const id = `genrel-${randomUUID()}`;
  db.prepare(`INSERT INTO cannabis_cultivar_genetic_relationships
...
export function listCultivarGeneticsAdmin(search = '') {
  ...
  const where = q ? `WHERE c.normalized_name LIKE ? OR EXISTS (SELECT 1 FROM cannabis_cultivar_aliases a WHERE a.cultivar_id=c.id AND a.normalized_alias LIKE ?)` : '';
  const params = q ? [`%${q}%`, `%${q}%`] : [];
  const cultivars = db.prepare(`
    ...
    FROM cannabis_cultivars c ${where}
    ORDER BY c.canonical_name COLLATE NOCASE LIMIT 500`).all(...params) as any[];
  const sources = db.prepare('SELECT * FROM cannabis_cultivar_sources ORDER BY verified DESC, source_name COLLATE NOCASE LIMIT 500').all() as any[];
  const lineage = db.prepare(`
    ...
    JOIN cannabis_cultivars child ON child.id=l.child_cultivar_id
    JOIN cannabis_cultivars parent ON parent.id=l.parent_cultivar_id
  ...
  const stats = {
    cultivars: Number((db.prepare('SELECT COUNT(*) AS n FROM cannabis_cultivars').get() as any)?.n || 0),
    aliases: Number((db.prepare('SELECT COUNT(*) AS n FROM cannabis_cultivar_aliases').get() as any)?.n || 0),
    lineageClaims: Number((db.prepare('SELECT COUNT(*) AS n FROM cannabis_cultivar_lineage_claims').get() as any)?.n || 0),
    sources: Number((db.prepare('SELECT COUNT(*) AS n FROM cannabis_cultivar_sources').get() as any)?.n || 0),
    productLinks: Number((db.prepare('SELECT COUNT(*) AS n FROM cannabis_product_cultivars').get() as any)?.n || 0),
    geneticRelationships: Number((db.prepare('SELECT COUNT(*) AS n FROM cannabis_cultivar_genetic_relationships').get() as any)?.n || 0),
  };
