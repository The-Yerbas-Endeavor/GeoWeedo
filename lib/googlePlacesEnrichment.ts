...
function placeholderName(v:unknown){return norm(v)==='data not available';}
...
async function search(base:any){
 ...
 const generic=placeholderName(base.name);
 const locality=[base.streetAddress,base.city,base.region,base.country].filter(Boolean).join(', ');
 const textQuery=generic?['cannabis dispensary',locality].filter(Boolean).join(', '):[base.name,locality].filter(Boolean).join(', ');
 const latitude=Number(base.latitude),longitude=Number(base.longitude),hasCoords=Number.isFinite(latitude)&&Number.isFinite(longitude);
 const body:any={textQuery,pageSize:5};
 if(generic&&hasCoords)body.locationBias={circle:{center:{latitude,longitude},radius:2000}};
 const response=await fetch('https://places.googleapis.com/v1/places:searchText',{method:'POST',headers:{'Content-Type':'application/json','X-Goog-Api-Key':key,'X-Goog-FieldMask':SEARCH_MASK},body:JSON.stringify(body),cache:'no-store',signal:AbortSignal.timeout(12000)});
...
function scoreMatch(base:any,p:PlaceLite){
 const generic=placeholderName(base.name);
 const name=generic?0:similarity(base.name,p.displayName?.text);
 const expected=[base.streetAddress,base.city,base.region].filter(Boolean).join(' ');
 const address=similarity(expected,p.formattedAddress);
 ...
 if(generic){
  const proximityPoints=distance==null?0:distance<=35?85:distance<=75?75:distance<=150?65:distance<=300?50:distance<=750?30:0;
  const score=Math.min(100,Math.round(proximityPoints+address*15));
  const confidence=score>70?'high':score>=50?'medium':'low';
  return{score,confidence,nameSimilarity:null,addressSimilarity:address,distanceMeters:distance,placeholderRecovery:true};
 }
...
export async function previewGooglePlacesEnrichment(locationId:string){
 ...
 const runnerUp=candidates[1]?.match.score||0;
 let confidence=best.match.score>70?'high':best.match.score>=50?'medium':'low';
 // Placeholder-name recovery relies heavily on coordinates. A near tie should
 // always go to Admin review rather than silently assigning the wrong business.
 if(placeholderName(base.name)&&runnerUp>0&&best.match.score-runnerUp<10)confidence='medium';
