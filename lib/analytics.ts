...
function hashListForRange(ips:string[],days:number){
 const values:string[]=[];
 const unique=Array.from(new Set(ips.map(ip=>ip.trim()).filter(Boolean)));
 for(let offset=0;offset<=days;offset++){
  const day=new Date(Date.now()-offset*86400000).toISOString().slice(0,10);
  for(const ip of unique)values.push(analyticsNetworkHashForDay(ip,day));
 }
...
function classifyBrowser(ua:string){
 if(/Edg\//i.test(ua))return 'Edge';if(/OPR\//i.test(ua))return 'Opera';if(/Firefox\//i.test(ua))return 'Firefox';if(/Chrome\//i.test(ua))return 'Chrome';if(/Safari\//i.test(ua)&&!/Chrome\//i.test(ua))return 'Safari';return 'Other';
}
...
export function analyticsSummary(days=30,filters:AnalyticsFilters={}){
 ...
 const hashes=hashListForRange(filters.excludeIps||[],range);
 ...
 const sessionPredicate=`s.started_at>=?${hashClause}${adminSessionClause}`;
 const sessionParams=[since,...hashes];
 const eventAdminClause=filters.excludeAdmin?` AND COALESCE(e.path,'/') NOT LIKE '/admin%'`:'';
 const eventPredicate=`e.created_at>=?${eventAdminClause} AND EXISTS (SELECT 1 FROM analytics_sessions s WHERE s.id=e.session_id${hashClause})`;
 const eventParams=[since,...hashes];
 const scalar=(sql:string,...params:any[])=>Number((database.prepare(sql).get(...params) as Record<string,unknown>|undefined)?.value||0);
 const totals={
  visitors:scalar(`SELECT COUNT(DISTINCT s.visitor_id) value FROM analytics_sessions s WHERE ${sessionPredicate}`,...sessionParams),
  sessions:scalar(`SELECT COUNT(*) value FROM analytics_sessions s WHERE ${sessionPredicate}`,...sessionParams),
  pageViews:scalar(`SELECT COUNT(*) value FROM analytics_events e WHERE e.event_type='page_view' AND ${eventPredicate}`,...eventParams),
  activeNow:scalar(`SELECT COUNT(*) value FROM analytics_sessions s WHERE s.last_seen_at>=?${hashClause}${adminSessionClause}`,oneMinuteAgo,...hashes),
  avgDurationMs:scalar(`SELECT COALESCE(AVG(e.duration_ms),0) value FROM analytics_events e WHERE e.event_type='page_leave' AND e.duration_ms IS NOT NULL AND ${eventPredicate}`,...eventParams),
  errors:scalar(`SELECT COUNT(*) value FROM analytics_events e WHERE e.event_type IN ('client_error','unhandled_rejection') AND ${eventPredicate}`,...eventParams),
 };
 ...
 const byDay=new Map<string,{day:string;page_views:number;visitors:number;sessions:number}>();
 for(let offset=range-1;offset>=0;offset--){const day=new Date(Date.now()-offset*86400000).toISOString().slice(0,10);byDay.set(day,{day,page_views:0,visitors:0,sessions:0});}
 for(const row of pageDaily){const target=byDay.get(String(row.day));if(target){target.page_views=Number(row.page_views||0);target.visitors=Number(row.visitors||0);}}
 for(const row of sessionDaily){const target=byDay.get(String(row.day));if(target)target.sessions=Number(row.sessions||0);}
 ...
 const devices=new Map<string,number>(),browsers=new Map<string,number>(),languages=new Map<string,number>(),timezones=new Map<string,number>();
 for(const row of sessionProfiles){
  const device=classifyDevice(Number(row.screen_width||0));devices.set(device,(devices.get(device)||0)+1);
  const browser=classifyBrowser(String(row.user_agent||''));browsers.set(browser,(browsers.get(browser)||0)+1);
  const language=String(row.language||'Unknown');languages.set(language,(languages.get(language)||0)+1);
  const timezone=String(row.timezone||'Unknown');timezones.set(timezone,(timezones.get(timezone)||0)+1);
 }
 const ranked=(map:Map<string,number>,key:string)=>Array.from(map.entries()).sort((a,b)=>b[1]-a[1]).slice(0,10).map(([name,sessions])=>({[key]:name,sessions}));
 const eventTypes=database.prepare(`SELECT e.event_type event_type,COUNT(*) events FROM analytics_events e WHERE ${eventPredicate} AND e.event_type NOT IN ('page_view','page_leave') GROUP BY e.event_type ORDER BY events DESC LIMIT 12`).all(...eventParams);
