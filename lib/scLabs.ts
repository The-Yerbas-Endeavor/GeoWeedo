...
export function ingestScLabsSample(sample: ScLabsNormalizedSample) {
  ...
  db.prepare(`INSERT INTO cannabis_coa_sources (id,batch_id,source_type,source_name,source_url,external_id,raw_payload_json,parser_version,fetched_at,verified,evidence_status,evidence_reason,created_at) VALUES (?,?, 'lab_public_page','SC Labs',?,?,?,?,?,1,'verified','official_lab_lookup',?)`).run(`coa-${crypto.randomUUID()}`,batchId,sample.sourceUrl,sample.sampleId,JSON.stringify(sample.raw),'sclabs-public-v2',now,now);
  return { productId: product.id, batchId, analyteCount: sample.analytes.length, sampleId: sample.sampleId, created: !existing };
}
