...
function normReviewName(value:unknown){return String(value||'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();}
function reviewDistanceMeters(aLat:number,aLng:number,bLat:number,bLng:number){const r=6371008.8,p=Math.PI/180,dLat=(bLat-aLat)*p,dLng=(bLng-aLng)*p,h=Math.sin(dLat/2)**2+Math.cos(aLat*p)*Math.cos(bLat*p)*Math.sin(dLng/2)**2;return r*2*Math.atan2(Math.sqrt(h),Math.sqrt(1-h));}
function resolveReviewLocationId(row:any,payload:any){
 const requested=String(row.location_id||'');
 if(requested&&getLocationBase(requested))return requested;
 const db=getDatabase();
 const records=[
  ...(db.prepare(`SELECT id,name,latitude,longitude FROM dispensaries`).all() as any[]),
  ...(db.prepare(`SELECT id,name,latitude,longitude FROM dispensary_candidates WHERE status<>'rejected'`).all() as any[]),
 ];
 const wanted=normReviewName(row.location_name);
 const exact=records.filter(record=>wanted&&normReviewName(record.name)===wanted);
 ...
 if(Number.isFinite(lat)&&Number.isFinite(lng)){
  ...
  const nearby=pool.map(record=>({record,distance:reviewDistanceMeters(lat,lng,Number(record.latitude),Number(record.longitude))})).filter(item=>Number.isFinite(item.distance)&&item.distance<=75).sort((a,b)=>a.distance-b.distance);
  if(nearby.length===1||(nearby.length>1&&nearby[0].distance+25<nearby[1].distance))return String(nearby[0].record.id);
 }
 return null;
}
...
export function listReviewQueue(status='pending',limit=100){ensureSchema();return (getDatabase().prepare(`SELECT * FROM dispensary_enrichment_review_queue WHERE status=? ORDER BY created_at ASC LIMIT ?`).all(status,Math.min(250,Math.max(1,limit))) as any[]).map((r:any)=>{const payload=JSON.parse(r.payload_json||'{}'),resolvedId=resolveReviewLocationId(r,payload),base=resolvedId?getLocationBase(resolvedId):null,profile=base&&resolvedId?profileState(resolvedId,base):null;return{...r,resolved_location_id:resolvedId,payload,existing:base?{name:base.name,streetAddress:base.streetAddress,city:base.city,region:base.region,postalCode:base.postalCode,country:base.country,latitude:base.latitude,longitude:base.longitude,phone:profile?.phone||base.phone||'',website:profile?.website||base.website||'',hours:profile?.hours||{}}:null};});}
export async function reviewQueueItem(reviewId:string,action:'approve'|'reject',actorId:string){ensureSchema();const db=getDatabase(),row=db.prepare(`SELECT * FROM dispensary_enrichment_review_queue WHERE id=? AND status='pending'`).get(reviewId) as any;if(!row)throw new Error('Review item not found or already reviewed.');const payload=JSON.parse(row.payload_json||'{}'),stamp=now();if(action==='approve'){const locationId=resolveReviewLocationId(row,payload);if(!locationId)throw new Error('The original enrichment location no longer exists. Reject this stale review or rerun enrichment for the current location.');if(locationId!==String(row.location_id)){db.prepare(`UPDATE dispensary_enrichment_review_queue SET location_id=?,updated_at=? WHERE id=?`).run(locationId,stamp,reviewId);db.prepare(`UPDATE dispensary_batch_items SET location_id=?,updated_at=? WHERE id=?`).run(locationId,stamp,row.item_id);}if(row.review_type==='google_places'){await applyGooglePlacesEnrichment(locationId,actorId,{...payload,confidence:'high'});db.prepare(`UPDATE dispensary_batch_items SET status='applied',stage='complete',message='Google Places match approved by admin.',result_json=?,updated_at=? WHERE id=?`).run(JSON.stringify({googlePlaces:payload}),stamp,row.item_id);}else{await enrichFromOfficialWebsite(locationId,actorId,true);db.prepare(`UPDATE dispensary_batch_items SET status='applied',stage='complete',message='Website enrichment approved by admin.',result_json=?,updated_at=? WHERE id=?`).run(JSON.stringify(payload),stamp,row.item_id);}}else db.prepare(`UPDATE dispensary_batch_items SET status='skipped',message='Rejected in enrichment review.',updated_at=? WHERE id=?`).run(stamp,row.item_id);db.prepare(`UPDATE dispensary_enrichment_review_queue SET status=?,reviewed_by=?,reviewed_at=?,updated_at=? WHERE id=?`).run(action==='approve'?'approved':'rejected',actorId,stamp,stamp,reviewId);return{ok:true};}