...
export function createBatchJob(actorId:string,scope:Scope,autoApply=true){
 ensureSchema();
 const db=getDatabase(),id=`batch-${crypto.randomUUID()}`,stamp=now(),canDiscover=discoveryConfigured(),canGoogle=googlePlacesConfigured();
 ...
 try{
  ...
  const pending=db.prepare(`INSERT INTO dispensary_batch_items(id,job_id,location_id,location_name,record_type,status,stage,message,created_at,updated_at) VALUES(?,?,?,?,?,'pending',?,NULL,?,?)`);
  ...
  for(const r of records){const locationId=String(r.id),state=profileState(locationId,r),itemId=`batch-item-${crypto.randomUUID()}`;if(!state.website&&!canDiscover&&!canGoogle)blocked.run(itemId,id,locationId,String(r.name),String(r.record_type),DISCOVERY_BLOCKED_MESSAGE,stamp,stamp);else pending.run(itemId,id,locationId,String(r.name),String(r.record_type),canGoogle?'google_places':'discovery',stamp,stamp);}
  db.exec('COMMIT');
...
async function processItem(item:any,job:any,actorId:string){
 ...
 try{
  let base=getLocationBase(locationId);if(!base)throw new Error('Location no longer exists.');
  if(googlePlacesConfigured()){
   const google=await previewGooglePlacesEnrichment(locationId);
   if(google.confidence==='high'){
    if(autoApply){await applyGooglePlacesEnrichment(locationId,actorId,google);base=getLocationBase(locationId)||base;}
    else{queueReview(job.id,item,'google_places','high',google,Number(google.score||0));return;}
   }else if(google.confidence==='medium'){
    queueReview(job.id,item,'google_places','medium',google,Number(google.score||0));return;
   }
   // Low-confidence/no Google match is never applied. Continue with existing official-site enrichment.
  }
  const profile=getCommunityProfile(locationId);let website=profile?.website||base.website||'';
  if(!website){
   ...
   if(!autoApply||!discovery.canAutoApply||!discovery.applied){queueReview(job.id,item,'discovery',selected.confidence||'low',discovery,Number(selected.score||0));return;}
   website=selected.url||'';
  }
  const preview=await enrichFromOfficialWebsite(locationId,actorId,false);const reviewPayload=enrichmentReviewPayload(locationId,base,preview);
  if(preview.confidence!=='high'){queueReview(job.id,item,'enrichment',preview.confidence,reviewPayload);return;}
  if(autoApply)await enrichFromOfficialWebsite(locationId,actorId,true);else{queueReview(job.id,item,'enrichment','high',reviewPayload);return;}
  db.prepare(`UPDATE dispensary_batch_items SET status='applied',stage='complete',message='High-confidence Google/site enrichment applied.',result_json=?,updated_at=? WHERE id=?`).run(JSON.stringify(reviewPayload),now(),item.id);
