...
export function createBatchJob(actorId:string,scope:Scope,autoApply=true){
 ...
 try{
  ...
  for(const r of records){const locationId=String(r.id),state=profileState(locationId,r),itemId=`batch-item-${crypto.randomUUID()}`;if(!state.website&&!canDiscover)blocked.run(itemId,id,locationId,String(r.name),String(r.record_type),DISCOVERY_BLOCKED_MESSAGE,stamp,stamp);else pending.run(itemId,id,locationId,String(r.name),String(r.record_type),stamp,stamp);}
  db.exec('COMMIT');
...
async function processItem(item:any,job:any,actorId:string){
 ...
 try{
  ...
  db.prepare(`UPDATE dispensary_batch_items SET status='applied',stage='complete',message='High-confidence site and enrichment applied.',result_json=?,updated_at=? WHERE id=?`).run(JSON.stringify(reviewPayload),now(),item.id);
