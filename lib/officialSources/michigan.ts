...
const HOSTS=['https://aca-prod.accela.com','https://aca3.accela.com'];
...
async function downloadDataset(host:string,dataset:Dataset){const url=`${host}/MIMM/Cap/CapHome.aspx?TabName=${dataset.tab}&module=${dataset.module}`;let cookies:string[]=[];const initial=await request(url,{headers:{Accept:'text/html,application/xhtml+xml'}},cookies);cookies=mergeCookies(cookies,initial);if(!initial.ok)throw new Error(`${dataset.label} initial Accela page returned ${initial.status}`);const initialHtml=await initial.text();if(!/__VIEWSTATE|ACA_CS_FIELD/i.test(initialHtml))throw new Error(`${dataset.label} Accela page did not expose a searchable public form`);
 ...
 const search=await request(url,{method:'POST',headers:{Accept:'text/html,application/xhtml+xml','Content-Type':'application/x-www-form-urlencoded',Origin:host,Referer:url},body:form.toString()},cookies);cookies=mergeCookies(cookies,search);if(!search.ok)throw new Error(`${dataset.label} blank Accela search returned ${search.status}`);const searchHtml=await search.text();const action=downloadAction(searchHtml);if(!action)throw new Error(`${dataset.label} blank Accela search returned results but no recognizable Download results control`);
...
export async function fetchMichiganCandidates():Promise<Row[]>{const results=await Promise.allSettled(DATASETS.map(dataset=>officialDownload(dataset)));const failures=results.map((result,index)=>result.status==='rejected'?`${DATASETS[index].label}: ${result.reason instanceof Error?result.reason.message:String(result.reason)}`:'').filter(Boolean);if(failures.length)throw new Error(`Michigan CRA official Accela license-download workflow failed. ${failures.join(' | ')}`);
 const rows:Row[]=[];for(let index=0;index<DATASETS.length;index++){const dataset=DATASETS[index],records=(results[index] as PromiseFulfilledResult<Record<string,string>[]>).value;for(const record of records){const status=pick(record,['Status','Record Status','License Status']);if(!/^active$/i.test(status))continue;const licenseNumber=pick(record,['Record Number','License Number','License #','Record #']);const recordType=pick(record,['Record Type','License Type','Type']);if(!dataset.accept(recordType,licenseNumber))continue;const name=pick(record,['Licensee Name','License Name','Business Name','Name','DBA Name']);const address=pick(record,['Address','Street Address','Facility Address','Physical Address']);if(!name||!address||!licenseNumber)continue;const parsed=parseAddress(address);rows.push({name,streetAddress:parsed.streetAddress,city:parsed.city,region:'Michigan',country:'USA',licenseNumber,dataSource:'Michigan CRA Accela License Download',sourceUrl:SOURCE,sourceLicense:'Official Michigan Cannabis Regulatory Agency Verify a License database export; Active adult-use Marijuana Retailer and medical Provisioning Center license records only.',imageryStatus:'missing_coordinates'});}}
 const unique=new Map<string,Row>();for(const row of rows)if(!unique.has(row.licenseNumber!.toLowerCase()))unique.set(row.licenseNumber!.toLowerCase(),row);if(!unique.size)throw new Error('Michigan CRA official Accela downloads completed but yielded zero Active Marijuana Retailer or Provisioning Center records; refusing an unverified import.');return Array.from(unique.values());}
