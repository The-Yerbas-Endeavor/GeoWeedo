...
export type DispensaryCandidate = {
  id: string; name: string; streetAddress?: string; city?: string; region?: string; postalCode?: string; country?: string;
...
export async function importCandidates(rows:Omit<DispensaryCandidate,'id'|'status'|'createdAt'|'updatedAt'>[]){
 ...
 const insert=db.prepare(`INSERT OR IGNORE INTO dispensary_candidates (id,fingerprint,name,street_address,city,region,postal_code,country,latitude,longitude,website,license_number,data_source,source_url,source_license,status,imagery_status,imagery_count,imagery_checked_at,imagery_message,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`);
 ...
 const fillBlanks=db.prepare(`UPDATE dispensary_candidates SET
  ...
  postal_code=CASE WHEN postal_code IS NULL OR trim(postal_code)='' THEN COALESCE(?,postal_code) ELSE postal_code END,
 ...
 for(const row of rows){
  ...
  if(existing){
   ...
   const fillable=[
    [existing.street_address,row.streetAddress],[existing.city,row.city],[existing.region,row.region],[existing.postal_code,row.postalCode],[existing.country,row.country],
