...
function findExistingBatch(db:any,productId:string,input:{uid?:string|null;batchNumber?:string|null;sampleId?:string|null}){
  if(input.uid){const row=db.prepare(`SELECT * FROM cannabis_batches WHERE uid=? LIMIT 1`).get(input.uid) as any;if(row){if(row.product_id!==productId)throw new Error(`UID ${input.uid} is already linked to another product.`);return row;}}
  if(input.sampleId){const row=db.prepare(`SELECT * FROM cannabis_batches WHERE coa_number=? LIMIT 1`).get(input.sampleId) as any;if(row){if(row.product_id!==productId)throw new Error(`COA/sample ${input.sampleId} is already linked to another product.`);return row;}}
  if(input.batchNumber){const rows=db.prepare(`SELECT * FROM cannabis_batches WHERE product_id=? AND batch_number=? LIMIT 2`).all(productId,input.batchNumber) as any[];if(rows.length===1)return rows[0];if(rows.length>1)throw new Error(`Batch ${input.batchNumber} is ambiguous for this product.`);}
  return null;
}

export function getAdminCoaReviewSubmissions(status='pending'){
  ensureWeedoMenuSchema(); const db=getDatabase();
  const rows=db.prepare(`SELECT s.*,u.username,u.display_name AS submitter_display_name,d.name AS dispensary_name,d.city AS dispensary_city,d.region AS dispensary_region,cu.id AS coa_upload_id,cu.sha256 AS coa_sha256,cu.original_filename AS coa_filename,cu.parsed_json AS coa_parsed_json,cu.status AS coa_upload_status,cu.created_at AS coa_uploaded_at FROM cannabis_product_submissions s JOIN users u ON u.id=s.submitted_by_user_id LEFT JOIN dispensaries d ON d.id=s.dispensary_id LEFT JOIN cannabis_coa_uploads cu ON cu.submission_id=s.id WHERE (?='all' OR s.status=?) ORDER BY CASE WHEN cu.id IS NULL THEN 1 ELSE 0 END,s.created_at ASC LIMIT 250`).all(status,status) as any[];
  return rows.map(row=>({...row,requested_menu_add:Boolean(row.requested_menu_add),menu_item:parseJson(row.menu_item_json),coa:row.coa_upload_id?{id:row.coa_upload_id,sha256:row.coa_sha256,filename:row.coa_filename,status:row.coa_upload_status,uploadedAt:row.coa_uploaded_at,parsed:parseJson(row.coa_parsed_json)||{}}:null}));
}

export async function approveExactBatchFromCoa(input:{submissionId:string;adminId:string;reviewNotes?:string|null}){
  ensureWeedoFactsSchema(); ensureWeedoMenuSchema(); const db=getDatabase();
  ...
  if(!upload.sha256)throw new Error('COA evidence hash is missing.');
  ...
  try{
    ...
    for(const analyte of analytes){
      const groupName=String(analyte?.groupName||'').trim(),analyteName=String(analyte?.analyteName||'').trim(); if(!groupName||!analyteName)continue;
      ...
      db.prepare(`INSERT INTO cannabis_analytes (id,batch_id,group_name,analyte_name,value,unit,lod,loq,status,limit_value,limit_unit,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`).run(`ca-${randomUUID()}`,batchId,groupName,analyteName,numberOrNull(analyte.value),analyte.unit||null,numberOrNull(analyte.lod),numberOrNull(analyte.loq),normalizeStatus(analyte.status),numberOrNull(analyte.limitValue),analyte.limitUnit||null,now);
    }
    ...
    if(existingSource)db.prepare(`UPDATE cannabis_coa_sources SET batch_id=?,source_type='lab_coa_pdf',source_name='SC Labs',source_url=?,raw_payload_json=?,parser_version='sclabs-coa-pdf-v2',fetched_at=?,verified=0,evidence_status='review',evidence_reason='uploaded_lab_document',document_path=?,document_sha256=?,document_mime_type='application/pdf',document_size=?,archived_at=? WHERE id=?`).run(batchId,sourceUrl,rawPayload,now,storedPath,upload.sha256,Number(upload.byte_size||fs.statSync(storedPath).size),upload.archived_at||now,existingSource.id);
    else db.prepare(`INSERT INTO cannabis_coa_sources (id,batch_id,source_type,source_name,source_url,external_id,raw_payload_json,parser_version,fetched_at,verified,evidence_status,evidence_reason,document_path,document_sha256,document_mime_type,document_size,archived_at,created_at) VALUES (?,?,'lab_coa_pdf','SC Labs',?,?,?,?,?,0,'review','uploaded_lab_document',?,?,'application/pdf',?,?,?)`).run(`coas-${randomUUID()}`,batchId,sourceUrl,externalId,rawPayload,'sclabs-coa-pdf-v2',now,storedPath,upload.sha256,Number(upload.byte_size||fs.statSync(storedPath).size),upload.archived_at||now,now);
