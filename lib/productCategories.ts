...
function seedCategories(db: Db) {
  ...
  for (const category of CATEGORY_SEEDS) {
    upsertCategory.run(category.id, category.slug, category.name, null, category.sort, now, now);
    for (const alias of category.aliases) {
      const normalized = normalize(alias);
      upsertAlias.run(`alias-${category.id}-${normalized.replace(/\s+/g, '-')}`, category.id, alias, normalized, '*', now);
    }
  }
}
...
export function ensureProductCategorySchema(db: Db = getDatabase()) {
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS cannabis_product_categories_active_idx
      ON cannabis_product_categories(active, sort_order, name);
    CREATE INDEX IF NOT EXISTS cannabis_product_category_aliases_category_idx
      ON cannabis_product_category_aliases(category_id, normalized_alias);
  `);
  ...
  if (tableExists(db, 'cannabis_products')) {
    db.exec('CREATE INDEX IF NOT EXISTS cannabis_products_category_idx ON cannabis_products(category_id)');
  }
  if (tableExists(db, 'dispensary_menu_items')) {
    db.exec('CREATE INDEX IF NOT EXISTS dispensary_menu_items_category_id_idx ON dispensary_menu_items(category_id, active)');
  }
...
export function resolveProductCategory(raw: unknown, db: Db = getDatabase()): ProductCategory | null {
  ...
  const exact = db.prepare(`
    ...
    ORDER BY CASE WHEN a.source_scope='*' THEN 0 ELSE 1 END, c.sort_order
    LIMIT 1
  ...
  if (!hit) return null;
  return { id: hit.id, slug: hit.slug, name: hit.name, description: hit.description, sort_order: hit.sort_order, active: hit.active };
}
...
export function backfillProductCategories(db: Db = getDatabase()) {
  ...
  for (const row of productRows) {
    const category = resolveProductCategory(row.product_type, db);
  ...
  if (tableExists(db, 'dispensary_menu_items')) {
    const menuRows = db.prepare(`
      SELECT mi.id,mi.category,mi.product_id,p.category_id AS product_category_id
      FROM dispensary_menu_items mi
      LEFT JOIN cannabis_products p ON p.id=mi.product_id
      WHERE mi.category_id IS NULL
    `).all() as Array<{ id: string; category: string | null; product_id: string | null; product_category_id: string | null }>;
    ...
    for (const row of menuRows) {
      const category = resolveProductCategory(row.category, db);
