...
export default function AdminTrafficOverview({totals,daily,excludedMatchedSessions=0,days,excludeAdmin,excludedIps}:Props){
 const style={'--traffic-accent':'var(--accent)'} as CSSProperties;
 return <section className="traffic-overview-shell" style={style}>
  <div className="traffic-overview-topline"><div><span className="eyebrow">FIRST-PARTY ANALYTICS</span><h2>Traffic overview</h2><p>At-a-glance site activity and individual users. Trends compare the newer half of the selected range with the earlier half.</p></div>{excludedMatchedSessions>0&&<div className="traffic-overview-status"><span className="traffic-excluded-pill">{excludedMatchedSessions.toLocaleString()} excluded</span></div>}</div>
  <div className="traffic-kpi-grid">
   ...
   <MetricCard label="Individual users" value={totals.visitors.toLocaleString()} metric="visitors" daily={daily} sub="Persistent visitors in range"/>
  ...
  <AdminIndividualVisitors days={days} excludeAdmin={excludeAdmin} excludedIps={excludedIps}/>
 </section>;
}
