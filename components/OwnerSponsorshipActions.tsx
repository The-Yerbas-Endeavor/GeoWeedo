...
const GAME_NAMES={classic:'Classic GeoWeedo',daily:'Daily Weedo',hunt:'Weedo Hunt'} as const;
...
export default function OwnerSponsorshipActions({locationId,featured,plan,requests,onChanged}:Props){
  ...
  const[gameType,setGameType]=useState<'classic'|'daily'|'hunt'>('classic');
  ...
  return <div className="owner-sponsor-actions">
    ...
    {showGame&&!pendingGame?<div className="owner-game-request">
      <div className="owner-game-request-grid">
        <label>Game<select value={gameType} onChange={event=>setGameType(event.target.value as typeof gameType)}><option value="classic">Classic GeoWeedo</option><option value="daily">Daily Weedo</option><option value="hunt">Weedo Hunt</option></select></label>
