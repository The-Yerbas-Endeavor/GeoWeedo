...
type GameType='classic'|'daily'|'hunt'|'mission';
...
function gameLabel(game:GameType){return game==='classic'?'Classic GeoWeedo':game==='daily'?'Daily GeoWeedo':game==='hunt'?'GeoWeedo Hunt':'Sponsored Mission';}
const DEFAULT_PRODUCTS:GameProducts={classic:{name:'Classic Sponsor',dayPriceCents:1000,weekPriceCents:4900,monthPriceCents:14900},daily:{name:'Daily GeoWeedo Sponsor',dayPriceCents:1500,weekPriceCents:7900,monthPriceCents:24900},hunt:{name:'Sponsored GeoWeedo Hunt',weekPriceCents:9900,monthPriceCents:29900},mission:{name:'Sponsored GeoWeedo Mission'}};
...
export default function AdminSponsorshipManager(){
 ...
 const[dispensarySearch,setDispensarySearch]=useState(''),[status,setStatus]=useState('Loading business sponsorship workspace…'),[busy,setBusy]=useState(false),[editorTab,setEditorTab]=useState<EditorTab>('featured');
 const names=useMemo(()=>new Map(dispensaries.map(item=>[item.id,item])),[dispensaries]);
 const choices=useMemo(()=>{const q=dispensarySearch.trim().toLowerCase();return q?dispensaries.filter(item=>`${item.name} ${item.city} ${item.region}`.toLowerCase().includes(q)):dispensaries;},[dispensaries,dispensarySearch]);
 const selectedFeatured=useMemo(()=>items.find(item=>item.dispensaryId===selectedDispensaryId),[items,selectedDispensaryId]);
 const selectedName=names.get(selectedDispensaryId)?.name||selectedDispensaryId||'No dispensary selected';
 ...
 function selectDispensary(id:string,loadExisting=true){
  if(!id)return;
  setSelectedDispensaryId(id);
  if(loadExisting){
   const existing=items.find(item=>item.dispensaryId===id);
   if(existing){setForm({source:existing.source,status:existing.status,startsAt:localInput(new Date(existing.startsAt)),endsAt:localInput(new Date(existing.endsAt))});}
 ...
 function handleSearch(value:string){
  ...
  if(!q)return;
  const matches=dispensaries.filter(item=>`${item.name} ${item.city} ${item.region}`.toLowerCase().includes(q));
  if(matches.length===1)selectDispensary(matches[0].id);
 }
 function editFeatured(item:Entitlement){setDispensarySearch('');selectDispensary(item.dispensaryId);setEditorTab('featured');window.scrollTo({top:0,behavior:'smooth'});}
 function editCampaign(item:Campaign){
  setDispensarySearch('');
  setSelectedDispensaryId(item.dispensaryId);
  setEditingCampaignId(item.id);
  setEditorTab('game');
  setCampaignForm({gameType:item.gameType,title:item.title||'',geographyType:item.geographyType||'all',geographyValue:item.geographyValue||'',radiusKm:item.radiusKm==null?'25':String(item.radiusKm),source:item.source||'admin_comp',status:item.status||'active',startsAt:localInput(new Date(item.startsAt)),endsAt:localInput(new Date(item.endsAt)),amountUsd:item.amountCents==null?'':String(item.amountCents/100)});
  setStatus(`Editing ${gameLabel(item.gameType)} sponsorship for ${names.get(item.dispensaryId)?.name||item.dispensaryId}.`);
  window.scrollTo({top:0,behavior:'smooth'});
 }
 ...
 const tabButton=(tab:EditorTab,label:string)=><button type="button" className={editorTab===tab?'primary':'ghost'} onClick={()=>setEditorTab(tab)} style={{padding:'9px 14px',minHeight:0}}>{label}</button>;
 ...
 return <main className="admin-shell">
  ...
  <section className="admin-panel sponsorship-compact" style={{padding:'18px 20px'}}>
   <div style={{display:'flex',justifyContent:'space-between',gap:18,alignItems:'flex-start',flexWrap:'wrap',marginBottom:12}}><div><h2 style={{margin:'0 0 4px'}}>GeoWeedo Featured</h2><p style={{margin:0,opacity:.72,fontSize:13}}>{money(plan.monthlyPriceCents)}/month · {money(plan.annualPriceCents)}/year · Featured + optional game sponsorships</p></div><div style={{display:'flex',gap:8}}>{tabButton('featured','Featured listing')}{tabButton('game','Game sponsorship')}</div></div>

   <div className="admin-form" style={{gap:10}}>
    <div className="field-row"><label><span>Search dispensaries</span><input type="search" value={dispensarySearch} onChange={e=>handleSearch(e.target.value)} placeholder="Search by name, city or state…"/></label><label><span>Dispensary</span><select value={selectedDispensaryId} onChange={e=>{setEditingCampaignId(null);selectDispensary(e.target.value);}}>{choices.length===0?<option value="">No matching dispensaries</option>:choices.map(item=><option value={item.id} key={item.id}>{item.name} — {item.city}, {item.region}</option>)}</select></label></div>
    {choices.length>1&&dispensarySearch.trim()&&<div style={{display:'flex',gap:6,flexWrap:'wrap'}}>{choices.slice(0,12).map(item=><button key={item.id} type="button" className={selectedDispensaryId===item.id?'primary':'ghost'} style={{padding:'6px 10px',minHeight:0}} onClick={()=>{setEditingCampaignId(null);selectDispensary(item.id);}}>{item.name} · {item.city}</button>)}</div>}

    <div style={{display:'flex',justifyContent:'space-between',gap:14,alignItems:'center',padding:'10px 12px',border:'1px solid rgba(255,255,255,.09)',borderRadius:12,background:'rgba(255,255,255,.025)',flexWrap:'wrap'}}><div><strong>Selected: {selectedName}</strong><div style={{fontSize:12,opacity:.72,marginTop:3}}>Current Featured status: {selectedFeatured?<><b style={{textTransform:'uppercase',color:selectedFeatured.status==='active'?'#67d66e':'inherit'}}>{selectedFeatured.status}</b> · {new Date(selectedFeatured.startsAt).toLocaleString()} → {new Date(selectedFeatured.endsAt).toLocaleString()}</>:'Not Featured'}</div></div>{selectedFeatured?.status==='active'?<button type="button" className="ghost" disabled={busy} onClick={()=>endSponsorship('featured',selectedFeatured.id,selectedName)}>End sponsorship</button>:null}</div>

    {editorTab==='featured'?<div style={{display:'grid',gap:10,paddingTop:6,borderTop:'1px solid rgba(255,255,255,.08)'}}>
     <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,flexWrap:'wrap'}}><div><strong>Featured listing</strong><div style={{fontSize:12,opacity:.68,marginTop:2}}>Enhanced pin, profile treatment, Browse priority and Featured analytics.</div></div>{quickButtons(p=>setForm(current=>({...current,endsAt:addDuration(current.startsAt,p)})))}</div>
     ...
     <button className="primary" disabled={busy||!selectedDispensaryId} onClick={saveFeatured}>Save Featured listing for {selectedName}</button>
    </div>:<div style={{display:'grid',gap:10,paddingTop:6,borderTop:'1px solid rgba(255,255,255,.08)'}}>
     <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,flexWrap:'wrap'}}><div><strong>{editingCampaignId?'Edit game sponsorship':product.name}</strong><div style={{fontSize:12,opacity:.68,marginTop:2}}>{editingCampaignId?'Update the active or scheduled campaign below. Changes take effect immediately after save.':`${priceLine} · Presented by placement · no change to mystery-location odds.`}</div></div><div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>{quickButtons(p=>setCampaignForm(current=>({...current,endsAt:addDuration(current.startsAt,p)})))}{editingCampaignId&&<button type="button" className="ghost" disabled={busy} onClick={cancelCampaignEdit}>Cancel edit</button>}</div></div>
     <div className="field-row"><label><span>Game</span><select value={campaignForm.gameType} onChange={e=>setCampaignForm({...campaignForm,gameType:e.target.value as GameType})}><option value="classic">Classic GeoWeedo</option><option value="daily">Daily GeoWeedo</option><option value="hunt">GeoWeedo Hunt</option><option value="mission">Sponsored Mission</option></select></label><label><span>Campaign title (optional)</span><input value={campaignForm.title} onChange={e=>setCampaignForm({...campaignForm,title:e.target.value})} placeholder="e.g. Green Gold California Hunt"/></label></div>
     ...
     <button className="primary" disabled={busy||!selectedDispensaryId} onClick={saveCampaign}>{editingCampaignId?`Update ${gameLabel(campaignForm.gameType)} sponsorship`:`Save ${gameLabel(campaignForm.gameType)} sponsorship for ${selectedName}`}</button>
    </div>}
   </div>

   <details style={{marginTop:12,borderTop:'1px solid rgba(255,255,255,.08)',paddingTop:10}}><summary style={{cursor:'pointer',fontWeight:800}}>Policy & current capabilities</summary><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:8,marginTop:10}}><div className="source-note"><strong>Classic GeoWeedo</strong><span>Presented by exposure and sponsor analytics. Sponsors never influence the five mystery Street View rounds.</span></div><div className="source-note"><strong>Daily GeoWeedo</strong><span>One overlapping active sponsor. Daily location remains deterministic and identical for every player.</span></div><div className="source-note"><strong>GeoWeedo Hunt</strong><span>Sponsored Hunt / Presented by promotion and analytics. Standard Hunt target selection remains independent.</span></div><div className="source-note"><strong>Sponsored Mission</strong><span>The sponsoring dispensary becomes the mission target. Players explore nearby Street View, guess the location, then get a branded sponsor reveal. Requires gameplay-ready Street View.</span></div><div className="source-note"><strong>Scheduling & billing</strong><span>Exact dates or 1 day, 1 week, 1 month and 1 year shortcuts. Admin comp, manual USD invoice or USD subscription.</span></div><div className="source-note"><strong>Geography</strong><span>All players, country, state/province, city/region or radius can be stored. Viewer-side geographic enforcement is not live yet.</span></div><div className="source-note"><strong>Analytics</strong><span>Game impressions/completions, sponsor profile/website activity and Featured map/profile analytics.</span></div></div></details>
  </section>

  <section className="admin-panel approved-list"><h2>Pending sponsorship requests</h2>
   {requests.filter(item=>item.status==='pending').length===0?<p>No pending sponsorship requests.</p>:requests.filter(item=>item.status==='pending').map(item=>{
    const d=names.get(item.dispensaryId);
    const description=item.requestType==='featured'
      ? 'Featured · '+(item.billingInterval==='annual'?'1 year':'1 month')
      : gameLabel(item.gameType||'classic')+' · '+(item.durationCode||'week')+' · '+(item.geographyType==='all'?'all players':item.geographyType==='radius'?(item.radiusKm||25)+' km radius':item.geographyType+': '+(item.geographyValue||'—'));
    return <div className="candidate-row" key={item.id}>
     <div><strong>{d?.name||item.dispensaryId}</strong><span>{description}</span><small>Requested {new Date(item.createdAt).toLocaleString()}{item.preferredStartAt?' · preferred start '+new Date(item.preferredStartAt).toLocaleDateString():''}{item.note?' · '+item.note:''}</small></div>
     <div className="candidate-actions"><span className="status-pill pending">pending</span><button disabled={busy} onClick={()=>loadRequest(item)}>Load request</button><button disabled={busy} onClick={()=>void rejectRequest(item.id)}>Reject</button></div>
    </div>;
   })}
  </section>
  <section className="admin-panel approved-list"><h2>Game sponsorships</h2>{campaigns.length===0?<p>No game sponsorships yet. Use the GeoWeedo Featured panel above to create a 1-day Admin comp test.</p>:campaigns.map(item=>{const d=names.get(item.dispensaryId);const geo=item.geographyType==='all'?'all players':item.geographyType==='radius'?`${item.radiusKm} km radius`:`${item.geographyType}: ${item.geographyValue||'—'}`;const editable=item.status==='active'||item.status==='paused';return <div className="candidate-row" key={item.id}><div><strong>{gameLabel(item.gameType)} · {item.title||d?.name||item.dispensaryId}</strong><span>{d?.name||item.dispensaryId} · {new Date(item.startsAt).toLocaleString()} → {new Date(item.endsAt).toLocaleString()}</span><small>{geo} · {item.source.replaceAll('_',' ')}{item.amountCents!=null?` · ${money(item.amountCents)}`:''}</small></div><div className="candidate-actions"><span className={`status-pill ${item.status}`}>{item.status}</span>{editable&&<button disabled={busy} onClick={()=>editCampaign(item)}>Edit</button>}{editable?<button disabled={busy} onClick={()=>endSponsorship('campaign',item.id,`${gameLabel(item.gameType)} · ${d?.name||item.dispensaryId}`)}>End sponsorship</button>:null}</div></div>;})}</section>
  ...
  <section className="admin-panel approved-list"><h2>All Featured listings</h2>{items.length===0?<p>No Featured listings yet.</p>:items.map(item=>{const d=names.get(item.dispensaryId),label=d?.name||item.dispensaryId;return <div className="candidate-row" key={item.id}><div><strong>{label}</strong><span>{plan.name} · USD · {new Date(item.startsAt).toLocaleDateString()} → {new Date(item.endsAt).toLocaleDateString()}</span><small>{item.source.replaceAll('_',' ')} · business {item.businessId}</small></div><div className="candidate-actions"><span className={`status-pill ${item.status}`}>{item.status}</span><button disabled={busy} onClick={()=>editFeatured(item)}>Edit</button>{item.status==='active'?<button disabled={busy} onClick={()=>endSponsorship('featured',item.id,label)}>End sponsorship</button>:null}</div></div>;})}</section>
 </main>;
}
