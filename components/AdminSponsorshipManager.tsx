...
export default function AdminSponsorshipManager() {
  ...
  const [status, setStatus] = useState('Unlock to manage YERB-sponsored placements.');
  ...
  async function load() {
    ...
    const response = await fetch('/api/admin/sponsorships', { headers }); const data = await response.json();
    ...
    setStatus('Sponsorship dashboard unlocked. Active campaigns can influence at most one round per standard game.');
  }

  async function save() {
    setBusy(true);
    try {
      const response = await fetch('/api/admin/sponsorships', { method: 'POST', headers: { ...headers, 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, amountYerb: Number(form.amountYerb), priorityWeight: Number(form.priorityWeight), startsAt: new Date(form.startsAt).toISOString(), endsAt: new Date(form.endsAt).toISOString() }) });
      const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Could not save sponsorship.');
      setStatus(`Saved sponsorship ${data.sponsorship.id}.`); await load();
  ...
  function edit(item: Sponsorship) {
    ...
    setStatus(`Loaded ${item.id}. Saving creates/updates the campaign values for this dispensary.`);
  }

  return (
    <main className="admin-shell">
      <header className="admin-header"><div><span className="eyebrow">GEOWEEDO ADMIN</span><h1>YERB sponsorships</h1></div><div className="admin-links"><a href="/admin/data">Data import</a><a href="/admin/dispensaries">Imagery</a><a href="/admin/rewards">Rewards</a><a href="/">Game</a></div></header>
      ...
      <section className="admin-grid">
        <div className="admin-panel"><h2>1. Admin access</h2><div className="field-row"><input type="password" value={secret} onChange={(e) => setSecret(e.target.value)} placeholder="GEOWEEDO_ADMIN_SECRET" /><button onClick={() => load().catch((e) => setStatus(e.message))}>Unlock</button></div>
          <h2>2. Campaign</h2><div className="admin-form">
            ...
            <button className="primary" disabled={busy || !secret || !form.dispensaryId} onClick={save}>Save sponsorship</button>
          </div>
        </div>
        <div className="admin-panel"><h2>Placement policy</h2><div className="source-note"><strong>One featured round maximum</strong><span>Standard games can include no more than one active sponsored dispensary.</span></div><div className="source-note"><strong>Daily challenge stays clean</strong><span>Daily competitive sets should be generated without sponsorship weighting.</span></div><div className="source-note"><strong>Score never changes</strong><span>Payment can affect selection frequency only, not answer, distance, points, or player YERB rate.</span></div><div className="source-note"><strong>On-chain receipt</strong><span>Store the YERB payment txid before switching a campaign to Active.</span></div></div>
      </section>
      <section className="admin-panel approved-list"><h2>Sponsorship ledger</h2>{items.length === 0 ? <p>No campaigns yet.</p> : items.map((item) => { const d = names.get(item.dispensaryId); return <div className="candidate-row" key={item.id}><div><strong>{d?.name || item.dispensaryId}</strong><span>{item.amountYerb} YERB · priority {item.priorityWeight} · {new Date(item.startsAt).toLocaleDateString()} → {new Date(item.endsAt).toLocaleDateString()}</span><small>{item.paymentTxid || 'Payment txid not recorded'}</small></div><div className="candidate-actions"><span className={`status-pill ${item.status}`}>{item.status}</span><button onClick={() => edit(item)}>Use as template</button></div></div>; })}</section>
    </main>
  );
}
