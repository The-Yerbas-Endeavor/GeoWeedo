    ...
    <main className="admin-shell">
      ...
      <section className="admin-grid">
        <div className="admin-panel"><h2>1. Admin access</h2><div className="field-row"><input type="password" value={secret} onChange={(e) => setSecret(e.target.value)} placeholder="GEOWEEDO_ADMIN_SECRET" /><button onClick={() => load().catch((e) => setStatus(e.message))}>Unlock</button></div>
          <h2>2. {editingId ? 'Edit campaign' : 'New campaign'}</h2><div className="admin-form">
            ...
            <div className="field-row"><button className="primary" disabled={busy || !secret || !form.dispensaryId} onClick={save}>{editingId ? 'Update sponsorship' : 'Create sponsorship'}</button><button disabled={!editingId} onClick={clearEdit}>Clear edit</button></div>
          </div>
        </div>
        <div className="admin-panel"><h2>Placement policy</h2><div className="source-note"><strong>One featured round maximum</strong><span>Standard games can include no more than one active sponsored dispensary.</span></div><div className="source-note"><strong>Daily challenge stays clean</strong><span>Daily competitive sets should be generated without sponsorship weighting.</span></div><div className="source-note"><strong>Score never changes</strong><span>Payment can affect selection frequency only, not answer, distance, points, or player YERB rate.</span></div><div className="source-note"><strong>On-chain receipt</strong><span>A YERB payment txid is required before a campaign can be Active.</span></div></div>
      </section>
