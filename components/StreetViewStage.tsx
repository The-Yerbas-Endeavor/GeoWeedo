...
export default function StreetViewStage({latitude,longitude,heading=0,photoId,imageryProvider='kartaview',imageUrl,projection='',fieldOfView=0}:Props){
 ...
 useEffect(()=>{const el=sphereRef.current;viewerRef.current?.destroy();viewerRef.current=null;if(!current||!isSphere||!el)return;const v=new Viewer({container:el,panorama:current.imageUrl,navbar:['zoom','move','fullscreen'],defaultYaw:((current.heading||0)*Math.PI)/180,defaultPitch:-0.16,mousemove:true,mousewheel:true,mousewheelCtrlKey:false,touchmoveTwoFingers:false,keyboard:'always',moveInertia:.9,moveSpeed:1.4});viewerRef.current=v;return()=>{if(viewerRef.current===v)viewerRef.current=null;v.destroy();};},[current,isSphere]);
 const step=(d:-1|1)=>{setFlatZoom(DEFAULT_FLAT_ZOOM);setIndex(v=>Math.min(Math.max(v+d,0),Math.max(0,photos.length-1)));};
 ...
 return <div ref={rootRef} className={`streetview-wrap ${isSphere?'streetview-spherical':'streetview-sequence'}`} data-imagery-provider={actualProvider}>
  ...
  {!loading&&current&&<>{isSphere?<><div ref={sphereRef} className="streetview-canvas interactive-sphere" tabIndex={0}/><div className="street-drag-hint">Drag to look around · wheel/pinch to zoom · WASD/arrows to look</div></>:<div className="street-photo-stage" tabIndex={0} onWheel={e=>{e.preventDefault();zoom(e.deltaY<0?.2:-.2);}}>
   <img src={current.imageUrl} alt={`${label} street imagery`} draggable={false} style={{transform:`scale(${flatZoom})`,transformOrigin:'50% 100%',objectFit:'cover',objectPosition:'50% 100%'}}/>
   ...
   <div className="street-zoom-controls"><button onClick={()=>zoom(-.25)} disabled={flatZoom<=1}>−</button><button onClick={()=>setFlatZoom(DEFAULT_FLAT_ZOOM)} disabled={flatZoom===DEFAULT_FLAT_ZOOM}>Reset</button><button onClick={()=>zoom(.25)} disabled={flatZoom>=3.5}>+</button></div>
