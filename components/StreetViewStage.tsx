...
export default function StreetViewStage({latitude,longitude,heading=0,photoId,imageryProvider='kartaview',imageUrl,projection='',fieldOfView=0}:Props){
 ...
 const step=(d:-1|1)=>{resetFlatView();setSkippingBadFrame(false);setIndex(v=>Math.min(Math.max(v+d,0),Math.max(0,photos.length-1)));};
 const zoom=(d:number)=>setFlatZoom(v=>Math.min(3.5,Math.max(1,v+d)));
 const skipFailedFlatFrame=()=>{
  if(!current)return;
  failedPhotoIdsRef.current.add(current.id);
  const nextForward=photos.findIndex((photo,i)=>i>index&&!failedPhotoIdsRef.current.has(photo.id));
  let nextIndex=nextForward;
  if(nextIndex<0){for(let i=index-1;i>=0;i--){if(!failedPhotoIdsRef.current.has(photos[i].id)){nextIndex=i;break;}}}
  if(nextIndex>=0&&nextIndex!==index){setSkippingBadFrame(true);resetFlatView();setIndex(nextIndex);return;}
 ...
 return <div ref={rootRef} className={`streetview-wrap ${isSphere?'streetview-spherical':'streetview-sequence'}`} data-imagery-provider={actualProvider}>
  ...
  {!loading&&current&&<>{isSphere?<><div ref={sphereRef} className="streetview-canvas interactive-sphere" tabIndex={0}/><div className="street-drag-hint">Drag to look around · wheel/pinch to zoom · WASD/arrows to look</div></>:<div className="street-photo-stage" tabIndex={0} onPointerDown={startFlatPan} onWheel={e=>{e.preventDefault();zoom(e.deltaY<0?.2:-.2);}} style={{cursor:flatPanning?'grabbing':'ns-resize',touchAction:'none'}}>
   <img key={current.id} src={current.imageUrl} alt={`${label} street imagery`} draggable={false} onLoad={()=>{setSkippingBadFrame(false);setError(null);}} onError={skipFailedFlatFrame} style={{transform:`translateY(${flatPanY}%) scale(${flatZoom})`,transformOrigin:'50% 100%',objectFit:'cover',objectPosition:'50% 100%',pointerEvents:'none',userSelect:'none'}}/>
  ...
  {!loading&&error&&<div className="map-error"><strong>Street View unavailable</strong><span>{error}</span></div>}
 </div>;
}
