...
export default function AdminDataManager() {
  ...
  const [status, setStatus] = useState('Loading official licensing data…');
  ...
  async function reject(item: Candidate) {
    const response = await fetch('/api/admin/candidates', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: item.id, status: 'rejected' }) });
    if (response.status === 401) { window.location.href = '/admin/login'; return; }
    await load();
  }
    ...
    <main className="admin-shell">
      ...
      <section className="admin-grid">
        <div className="admin-panel">
          <h2>Import licensing CSV / JSON</h2>
