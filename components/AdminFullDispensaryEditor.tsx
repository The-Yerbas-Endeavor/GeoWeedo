...
function isPlaceholderDispensary(r:Row){return r.kind==='dispensary'&&String(r.name||'').trim().replace(/\s+/g,' ').toLowerCase()==='data not available';}
...
export default function AdminFullDispensaryEditor(){
 ...
 const placeholderRows=useMemo(()=>filtered.filter(isPlaceholderDispensary),[filtered]);
 const repairablePlaceholderRows=useMemo(()=>placeholderRows.filter(hasCoordinates),[placeholderRows]);
 ...
 async function deleteUnusedDispensary(row:Row){if(row.kind!=='dispensary')return;if(row.active!==0||row.gameplay_enabled!==0){setStatus(row.name+': hide from browse and disable gameplay before deleting.');return;}if(!window.confirm('Permanently delete unused dispensary "'+row.name+'"? This cannot be undone. Records with game history, menus, owners, reviews, or sponsorship history are protected and will not be deleted.'))return;setBusyId(row.id);setStatus(row.name+': checking references and deleting…');try{const response=await fetch('/api/admin/dispensary-records',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:row.id,action:'delete-unused-dispensary'})});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||'Could not delete unused dispensary.');await reload(row.name+': unused dispensary deleted.',null);}catch(error){setStatus(error instanceof Error?error.message:'Could not delete unused dispensary.');}finally{setBusyId(null);}}
 ...
 async function repairPlaceholderRows(targets:Row[]){
  const rowsToRepair=targets.filter(row=>isPlaceholderDispensary(row)&&hasCoordinates(row));
  if(!rowsToRepair.length){setStatus('No Data Not Available dispensaries with valid coordinates are in the current selection.');return;}
  if(!window.confirm('Repair '+rowsToRepair.length+' Data Not Available dispensar'+(rowsToRepair.length===1?'y':'ies')+' from their saved location? High-confidence Google Places matches will update automatically; ambiguous matches will be sent to Admin review.'))return;
  setBulkBusy(true);
  let applied=0,review=0,failed=0;
  try{
   for(let i=0;i<rowsToRepair.length;i++){
    const row=rowsToRepair[i];
    setStatus('Repairing Data Not Available '+(i+1)+' of '+rowsToRepair.length+' · '+(row.city||normalizeState(row.region))+'…');
    try{
     const response=await fetch('/api/admin/dispensary-batch-enrichment',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'singleDispensary',locationId:row.id,autoApply:true})});
     const data=await response.json().catch(()=>({}));
    ...
    if(i<rowsToRepair.length-1)await new Promise(resolve=>setTimeout(resolve,250));
   }
   await reload('Placeholder repair complete: '+applied+' updated'+(review?' · '+review+' sent to review':'')+(failed?' · '+failed+' failed':'')+'.',null);
  }finally{setBulkBusy(false);}
 }
 async function deletePlaceholderRows(targets:Row[]){
  const ids=targets.filter(isPlaceholderDispensary).map(row=>row.id);
  if(!ids.length){setStatus('No Data Not Available dispensaries are in the current selection.');return;}
  const bulk=ids.length>1;
  const scopeText=bulk?ids.length+' Data Not Available dispensaries':'this Data Not Available dispensary';
  if(!window.confirm('Permanently delete '+scopeText+'? Records with game history, menus, owners, reviews, or sponsorship history will be protected and skipped.'))return;
  if(bulk)setBulkBusy(true);else setBusyId(ids[0]);
  setStatus('Deleting placeholder dispensar'+(bulk?'ies…':'y…'));
  try{
   const response=await fetch('/api/admin/dispensary-records',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'delete-placeholder-dispensaries',ids})});
   const data=await response.json().catch(()=>({}));
   if(!response.ok)throw new Error(data.error||'Placeholder deletion failed.');
   const deleted=Array.isArray(data.deleted)?data.deleted.length:0,skipped=Array.isArray(data.skipped)?data.skipped:[];
   await reload('Deleted '+deleted+' Data Not Available dispensar'+(deleted===1?'y':'ies')+(skipped.length?' · '+skipped.length+' protected/skipped.':'.'),null);
  }catch(error){setStatus(error instanceof Error?error.message:'Placeholder deletion failed.');}
  finally{if(bulk)setBulkBusy(false);else setBusyId(null);}
 }
 ...
 return <section className="admin-panel full-dispensary-editor unified-location-manager" style={{marginBottom:24}}>
  ...
  <div className="location-bulk-actions" style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap',marginTop:12,padding:'10px 12px',border:'1px solid var(--border)',borderRadius:10,background:'rgba(255,255,255,.02)'}}>
   ...
   {repairablePlaceholderRows.length?<button className="primary" type="button" disabled={bulkBusy} onClick={()=>void repairPlaceholderRows(repairablePlaceholderRows)}>Repair Data Not Available ({repairablePlaceholderRows.length})</button>:null}
     ...
     <table className="location-table">
      ...
      <tbody>{visible.map(row=>{const eligible=bulkGameplayEligible(row),checked=selectedKeySet.has(rowKey(row));return <tr key={row.kind+':'+row.id} onClick={()=>choose(row)} style={{cursor:'pointer',borderTop:'1px solid var(--border)',background:checked?'rgba(103,214,110,.05)':undefined}}>
       ...
       <td onClick={event=>event.stopPropagation()}><div className="location-actions">
        ...
        {row.kind==='dispensary'?<>{actionButton('Confirm',()=>void confirmProfile(row),busyId===row.id)}{actionButton('Enrich',()=>void enrichDispensary(row),busyId===row.id)}{actionButton(row.active===0?'Show browse':'Hide browse',()=>void locationAction(row,'set-browse',row.active===0),busyId===row.id)}{actionButton(row.gameplay_enabled===0?'Enable gameplay':'Disable gameplay',()=>void locationAction(row,'set-gameplay',row.gameplay_enabled===0),busyId===row.id||row.gameplay_enabled===0&&!gameReady(row))}{isPlaceholderDispensary(row)&&hasCoordinates(row)?actionButton('Repair name',()=>void repairPlaceholderRows([row]),busyId===row.id,true):null}{row.active===0&&row.gameplay_enabled===0&&!isPlaceholderDispensary(row)?actionButton('Delete unused',()=>void deleteUnusedDispensary(row),busyId===row.id):null}</>:row.status==='rejected'?<>{actionButton('Restore',()=>void setCandidateStatus(row,'candidate'),busyId===row.id,true)}{actionButton('Delete',()=>void deleteRejectedCandidate(row),busyId===row.id)}</>:<>{actionButton('Enrich',()=>void enrichCandidate(row),busyId===row.id)}{actionButton('Enable gameplay',()=>void confirmAndEnableCandidate(row),busyId===row.id||!hasCoordinates(row),true)}{actionButton('Reject',()=>void setCandidateStatus(row,'rejected'),busyId===row.id)}</>}
       </div></td>
      </tr>})}</tbody>
     </table>
    </div>
