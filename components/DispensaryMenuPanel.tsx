...
function factsHref(item: any) {
  if (!item.product_id) return '/weedo-facts';
  ...
  return <section className={styles.section}>
    ...
    {items.length === 0 ? <div className={styles.empty}>No GeoWeedo-linked menu items have been published for this dispensary yet.</div> : <div className={styles.list}>{items.map((item: any) => {
      ...
      return <article className={styles.item} key={item.id}>
        ...
        <div className={styles.links}>{item.product_id ? <a href={factsHref(item)}>View Nutritional Facts →</a> : <a href="/weedo-facts">Search Weedo Facts →</a>}{item.source_url ? <a href={item.source_url} target="_blank" rel="noreferrer">Menu source ↗</a> : null}</div>
      </article>;
    })}</div>}
  </section>;
}
