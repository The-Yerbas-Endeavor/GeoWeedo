...
export default function WeedoFactsLookup() {
  ...
  return (
    <div className={`weedoFactsLookup${result?.found ? ' has-result' : ''}`}>
      ...
      {result?.found && result.record ? <FactsCard record={result.record} scanId={result?.persistedQr?.id || null} /> : null}
    </div>
  );
}
...
function FactsCard({ record, scanId }: { record: any; scanId?: string | null }) {
  ...
  return (
    <article className="weedoFactsCard">
      ...
      {record.productId ? <WeedoFactsNearby productId={record.productId} batchId={record.batchId} productName={record.productName} reportSource="scanner" scanId={scanId} /> : null}
    </article>
  );
}
