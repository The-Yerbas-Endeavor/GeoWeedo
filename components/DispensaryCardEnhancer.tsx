...
function resolveCard(card:Element,pool:MapItem[]){
 ...
 const previousId=(card as HTMLElement).dataset.locationId,previousKind=(card as HTMLElement).dataset.locationKind as RecordKind|undefined;
 if(previousId&&previousKind){const previous=matches.find(item=>item.id===previousId&&item.kind===previousKind);if(previous)return previous;}
 return null;
}
...
export default function DispensaryCardEnhancer(){
 useEffect(()=>{
  ...
  async function enhance(card:HTMLElement){
   if(disposed)return;const match=resolveCard(card,pool);if(!match)return;const identity=`${match.kind}:${match.id}`;if(card.dataset.enhancedIdentity===identity&&card.querySelector('.map-community-summary'))return;
