...
function todayInfo(timeZone:string){const parts=new Intl.DateTimeFormat('en-US',{timeZone,weekday:'long',hour:'2-digit',minute:'2-digit',hourCycle:'h23'}).formatToParts(new Date());const weekday=parts.find(p=>p.type==='weekday')?.value||'';const hour=Number(parts.find(p=>p.type==='hour')?.value||0);const minute=Number(parts.find(p=>p.type==='minute')?.value||0);return{weekday,minutes:hour*60+minute};}
...
function rewriteInfoRows(card:Element,location:any){
 const dl=card.querySelector('dl');if(!dl)return;const address=[location.streetAddress,location.city,location.region,location.postalCode].filter(Boolean).join(', ');
 const addressRow=ensureInfoRow(dl,'Address','Coordinates');const addressDd=addressRow.querySelector('dd');if(addressDd)addressDd.textContent=address||'Address unavailable';
 const hoursRow=ensureInfoRow(dl,'Hours','Source');const hoursDd=hoursRow.querySelector('dd');if(hoursDd){hoursDd.textContent='';const zone=timezoneFor(location);let schedule='';let state:'open'|'closed'|'unknown'='unknown';if(zone){const local=todayInfo(zone);schedule=todaysHours(location.hours,local.weekday);state=openState(schedule,local.minutes);}const text=document.createElement('span');text.textContent=schedule||'Hours unavailable';hoursDd.appendChild(text);const badge=document.createElement('span');badge.className=`map-hours-badge ${state}`;badge.textContent=state==='open'?'Open':state==='closed'?'Closed':'Hours unavailable';hoursDd.appendChild(badge);}
}
