...
function ProductAwareMap(props:Props){
 ...
 useEffect(()=>{
  ...
  fetch(endpoint,{cache:'no-store',signal:controller.signal})
   ...
   .then(data=>{const next=Array.isArray(data.dispensaries)?data.dispensaries:[];const label=exactId?String(data.product?.label||data.product?.productName||'Selected product'):q;setResults(next);setResultQuery(label);if(exactId)setExactProductLabel(label);setLoading(false);rootRef.current?.querySelector<HTMLButtonElement>('.map-location-close')?.click();window.setTimeout(()=>window.dispatchEvent(new CustomEvent('geoweedo:open-browse-panel')),20);})
 ...
 const searchControl=toolbar?createPortal(<div className="map-unified-search" data-map-scanner-embedded="1" style={{display:'flex',alignItems:'center',gap:6,position:'relative',minWidth:0,flex:'1 1 300px',maxWidth:420}}>
   <div className="map-unified-search-shell" style={{position:'relative',display:'flex',alignItems:'center',width:'100%',minWidth:0}}>
    <input className="map-unified-search-input" value={inputValue} onPointerDown={event=>event.stopPropagation()} onMouseDown={event=>event.stopPropagation()} onClick={event=>event.stopPropagation()} onFocus={event=>event.stopPropagation()} onChange={event=>{if(exactProductId){setExactProductId('');setExactProductLabel('');clearProductParam();}setQuery(event.target.value);}} placeholder="Search dispensary, product, brand or ZIP" aria-label="Search dispensary, product, brand or ZIP" autoComplete="off" style={{width:'100%',minWidth:180,paddingRight:inputValue?82:48}}/>
