...
export default function GuessMap({guess,actual=null,revealed=false,onGuess,locations=[],browseMode=false}:Props){
  ...
  useEffect(()=>{const map=mapRef.current;if(!map||!browseMode||!mapReady)return;const markers=browseMarkerRefs.current;const syncPins=()=>{const visible=filteredLocations.filter(validLocation).sort((a,b)=>markerPriority(a)-markerPriority(b));const wanted=new Set(visible.map(locationIdentity));markers.forEach((marker,identity)=>{if(!wanted.has(identity)){marker.remove();markers.delete(identity);}});for(const item of visible){const identity=locationIdentity(item);if(markers.has(identity))continue;const marker=new Marker({color:markerColor(item),scale:markerScale(item)}).setLngLat([item.lng,item.lat]).setPopup(new Popup({offset:20,closeButton:false}).setText(item.name));const el=raiseMarker(marker,markerZIndex(item));el.title=[item.name,item.city,item.region].filter(Boolean).join(' · ');el.style.cursor='pointer';el.dataset.locationIdentity=identity;el.dataset.pinPriority=item.sponsored?'sponsored':item.approved&&item.imageryReady?'gameplay':'browse';el.addEventListener('click',event=>{event.stopPropagation();setSelectedLocation(item);setBrowserOpen(true);setStreetViewOpen(true);showSelectedMarker(item);});marker.addTo(map);raiseMarker(marker,markerZIndex(item));markers.set(identity,marker);}setBrowseLoadedCount(visible.length);};syncPins();return()=>{markers.forEach(marker=>marker.remove());markers.clear();};},[browseMode,filteredLocations,mapReady]);
