...
export default function GuessMap({guess,actual=null,revealed=false,onGuess,locations=[],browseMode=false}:Props){
  ...
  return <div className="guess-map-wrap">
    ...
    {browseMode&&browserOpen&&!selectedLocation&&<aside className="map-browser-panel" aria-label="Browse dispensaries"><div className="map-browser-panel-head"><div><span>BROWSE DISPENSARIES</span><strong>{browseList.length.toLocaleString()} mapped locations · {stateGroups.length} states</strong></div><button type="button" onClick={()=>setBrowserOpen(false)}>×</button></div><div className="map-browser-list">{browseList.length===0?<div className="map-browser-empty">No dispensaries match the active filters.</div>:stateGroups.map(group=>{const forcedOpen=(searchActive&&browseList.length<=MAX_AUTO_FIT_RESULTS)||region!=='all';const open=forcedOpen||Boolean(expandedStates[group.state]);return <section className="map-browser-state" key={group.state}><button type="button" className="map-browser-state-head" onClick={()=>toggleState(group.state,group.items)} aria-expanded={open}><span><strong>{group.state}</strong><small>{group.items.length.toLocaleString()} dispensaries</small></span><b>{open?'−':'+'}</b></button>{open&&renderStateRows(group.items,group.state)}</section>;})}</div></aside>}
