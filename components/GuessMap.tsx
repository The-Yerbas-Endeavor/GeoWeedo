...
function scopeVisible(item:MapLocation,scope:BrowseScope){if(scope==='enabled')return isPlayable(item);if(scope==='listed')return isListed(item);return true;}
...
export default function GuessMap({guess,actual=null,revealed=false,onGuess,locations=[],browseMode=false}:Props){
  ...
  const filteredLocations=useMemo(()=>locations.filter(i=>scopeVisible(i,browseScope)&&categoryVisible(i,showPlayable,showBrowse,showSponsored)&&(region==='all'||i.region===region)&&(!activeSearch||`${i.name} ${i.city||''} ${i.region||''}`.toLowerCase().includes(activeSearch))),[locations,browseScope,region,activeSearch,showPlayable,showBrowse,showSponsored]);
  ...
  const renderStateRows=(items:MapLocation[],state:string)=>{
    const play=items.filter(isPlayable);
    const listed=items.filter(item=>!isPlayable(item)&&isListed(item));
    const remaining=items.filter(item=>!isPlayable(item)&&!isListed(item));
    const ordered=[...play,...listed,...remaining].slice(0,MAX_RENDERED_ROWS_PER_STATE);
    const groupStart=(kind:'play'|'listed'|'all')=>kind==='play'?0:kind==='listed'?play.length:play.length+listed.length;
    return <div className="map-browser-state-list">{ordered.map((item,index)=>{const showPlayLabel=play.length>0&&index===groupStart('play'),showListedLabel=listed.length>0&&index===groupStart('listed'),showAllLabel=remaining.length>0&&index===groupStart('all');return <div key={locationIdentity(item)}>{showPlayLabel&&<div className="map-browser-tier-heading"><strong>Play</strong><small>{play.length.toLocaleString()} gameplay-ready</small></div>}{showListedLabel&&<div className="map-browser-tier-heading"><strong>Listed</strong><small>{listed.length.toLocaleString()} verified listings</small></div>}{showAllLabel&&<div className="map-browser-tier-heading map-browser-tier-heading-muted"><strong>All others</strong><small>{remaining.length.toLocaleString()} mapped locations</small></div>}<button type="button" className="map-browser-row" onClick={()=>focusLocation(item)}><span className="map-browser-row-pin">●</span><span className="map-browser-row-copy"><strong>{item.name}</strong><small>{item.city||state}</small></span><span className="map-browser-row-status">{item.sponsored?'★':isPlayable(item)?'PLAY':isListed(item)?'LISTED':'›'}</span></button></div>})}{items.length>ordered.length&&<div className="map-browser-empty">Showing first {ordered.length.toLocaleString()} of {items.length.toLocaleString()} results. Refine your search to narrow the list.</div>}</div>;
  };
  ...
  return <div className="guess-map-wrap">
    ...
    {browseMode&&<>
      <div className="map-browser-tools">
        ...
        <select value={browseScope} onChange={e=>setBrowseScope(e.target.value as BrowseScope)} aria-label="Filter dispensary status"><option value="all">All</option><option value="listed">Listed</option><option value="enabled">Enabled</option></select>
    ...
    {browseMode&&browserOpen&&!selectedLocation&&<aside className="map-browser-panel" aria-label="Browse dispensaries"><div className="map-browser-panel-head"><div><span>BROWSE DISPENSARIES</span><strong>{browseList.length.toLocaleString()} locations · {stateGroups.length} states</strong></div><button type="button" onClick={()=>setBrowserOpen(false)}>×</button></div><div className="map-browser-list">{browseList.length===0?<div className="map-browser-empty">No dispensaries match the active filters.</div>:stateGroups.map(group=>{const forcedOpen=(searchActive&&browseList.length<=MAX_AUTO_FIT_RESULTS)||region!=='all';const open=forcedOpen||Boolean(expandedStates[group.state]);return <section className="map-browser-state" key={group.state}><button type="button" className="map-browser-state-head" onClick={()=>toggleState(group.state,group.items)} aria-expanded={open}><span><strong>{group.state}</strong><small>{group.items.length.toLocaleString()} dispensaries</small></span><b>{open?'−':'+'}</b></button>{open&&renderStateRows(group.items,group.state)}</section>;})}</div></aside>}
