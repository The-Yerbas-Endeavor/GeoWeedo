...
type MapDispensary={id:string;slug?:string;name:string;latitude:number;longitude:number;city?:string;region?:string;country?:string;status?:string;imageryStatus?:string;imageryVerifiedAt?:string;panoramaId?:string;imageryPhotoId?:string;imageryUrl?:string};
...
export default function EnabledDispensaryBrowseFilter(){
 useEffect(()=>{
  let cancelled=false,scope:BrowseScope='all',timer:number|undefined,idleHandle:number|undefined,dataPromise:Promise<void>|null=null,dataReady=false;
  let listed:MapDispensary[]=[],playable:MapDispensary[]=[],mapped:MapDispensary[]=[];
  ...
  const applyMarkerScope=(items:MapDispensary[])=>{
   ...
   const ids=new Set(items.map(item=>String(item.id))),labels=new Set(items.map(item=>labelKey(item.name,item.city,item.region)));
   document.querySelectorAll<HTMLElement>('.maplibregl-marker[data-location-identity]').forEach(marker=>{
    ...
    const markerId=(marker.dataset.locationIdentity||'').split('|')[0],parts=(marker.title||'').split(' · '),show=ids.has(markerId)||labels.has(labelKey(parts[0],parts[1],parts[2]));
    if(show){const original=marker.dataset.enabledFilterOriginalDisplay;if(original==='__empty__')marker.style.removeProperty('display');else if(original!==undefined)marker.style.display=original;}else marker.style.setProperty('display','none','important');
   });
  };
  ...
  const makeScopedList=(section:HTMLElement,state:string,items:MapDispensary[])=>{
   ...
   const head=section.querySelector<HTMLElement>('.map-browser-state-head'),original=section.querySelector<HTMLElement>('.map-browser-state-list:not(.geoweedo-scope-generated)');
   if(scope==='all'){if(original)original.style.display='';return;}
   if(original)original.style.display='none';
   if(head?.getAttribute('aria-expanded')!=='true'||!items.length)return;
   ...
   for(const item of [...items].sort((a,b)=>(a.city||'').localeCompare(b.city||'')||a.name.localeCompare(b.name))){
    ...
    const status=document.createElement('span');status.className='map-browser-row-status';status.textContent=scope==='playable'?'PLAY':'LISTED';row.append(pin,copy,status);wrap.appendChild(row);
   }
  ...
  function apply(){
   ...
   document.querySelectorAll('.map-browser-scope-tabs').forEach(node=>node.remove());const control=ensureScopeSelect();if(control&&control.value!==scope)control.value=scope;
   const tools=document.querySelector<HTMLElement>('.map-first-home .map-browser-tools'),regionSelect=tools?.querySelector<HTMLSelectElement>('select[aria-label="Filter by state"]');
   const selectedRegion=String(regionSelect?.value||'all').trim(),regionFiltered=Boolean(selectedRegion&&selectedRegion!=='all'),panel=document.querySelector<HTMLElement>('.map-browser-panel');
   if(scope!=='all'&&!dataReady){void loadScopeData();const summary=panel?.querySelector<HTMLElement>('.map-browser-panel-head strong');if(summary)summary.textContent='Loading dispensary classification…';return;}if(!dataReady)return;
   const active=byRegion(scopedItems(),selectedRegion),mappedInScope=byRegion(mapped,selectedRegion),listedInScope=byRegion(listed,selectedRegion),playableInScope=byRegion(playable,selectedRegion);applyMarkerScope(scopedItems());
   document.querySelectorAll<HTMLElement>('.map-browser-state').forEach(section=>{
    const state=section.querySelector<HTMLElement>('.map-browser-state-head strong')?.textContent?.trim()||'',countNode=section.querySelector<HTMLElement>('.map-browser-state-head small');
    const mappedCount=mappedCounts.get(state)||0,listedCount=listedCounts.get(state)||0,playableCount=playableCounts.get(state)||0,regionMatches=!regionFiltered||state===selectedRegion,scopeCount=scope==='listed'?listedCount:scope==='playable'?playableCount:mappedCount;
    section.style.display=(!regionMatches||(scope!=='all'&&scopeCount===0))?'none':'';if(countNode)countNode.textContent=`${mappedCount.toLocaleString()} mapped · ${listedCount.toLocaleString()} listed · ${playableCount.toLocaleString()} playable`;
    makeScopedList(section,state,active.filter(item=>String(item.region||'').trim()===state));
   });
   const countries=new Set(active.map(item=>String(item.country||'USA').trim()||'USA')),summary=panel?.querySelector<HTMLElement>('.map-browser-panel-head strong');
   if(summary){if(regionFiltered)summary.textContent=`${mappedInScope.length.toLocaleString()} mapped · ${listedInScope.length.toLocaleString()} listed · ${playableInScope.length.toLocaleString()} playable · ${selectedRegion}`;else if(scope==='listed')summary.textContent=`${listedInScope.length.toLocaleString()} listed locations · ${countries.size.toLocaleString()} countr${countries.size===1?'y':'ies'}`;else if(scope==='playable')summary.textContent=`${playableInScope.length.toLocaleString()} playable locations · ${countries.size.toLocaleString()} countr${countries.size===1?'y':'ies'}`;}
   const existingEmpty=panel?.querySelector<HTMLElement>('.enabled-filter-empty');if(scope!=='all'&&active.length===0){if(!existingEmpty){const list=panel?.querySelector<HTMLElement>('.map-browser-list');if(list){const node=document.createElement('div');node.className='map-browser-empty enabled-filter-empty';node.textContent=scope==='listed'?'No listed dispensaries match the active filters.':'No playable dispensaries match the active filters.';list.appendChild(node);}}}else existingEmpty?.remove();
  }

  const onFilterChange=(event:Event)=>{const target=event.target as HTMLSelectElement|null;if(target?.matches('select[aria-label="Filter by state"]'))scheduleApply(0);};document.documentElement.dataset.geoweedoBrowseScope='all';apply();
  const idle=()=>{if(!cancelled)void loadScopeData();};if(typeof (window as any).requestIdleCallback==='function')idleHandle=(window as any).requestIdleCallback(idle,{timeout:1800});else idleHandle=window.setTimeout(idle,1100);
  const observer=new MutationObserver(mutations=>{const relevant=mutations.some(m=>Array.from(m.addedNodes).some(node=>node instanceof HTMLElement&&!node.classList.contains('geoweedo-scope-generated')&&(node.matches?.('.map-browser-tools,.map-browser-panel,.map-browser-state')||Boolean(node.querySelector?.('.map-browser-tools,.map-browser-panel,.map-browser-state')))));if(relevant)scheduleApply(100);});
