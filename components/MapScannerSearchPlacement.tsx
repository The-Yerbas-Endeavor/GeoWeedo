...
export default function MapScannerSearchPlacement() {
  ...
  const startBrowserScanner = useCallback(async () => {
    ...
    try {
      ...
      setScannerMessage('Point the camera at a QR code or barcode.');
      ...
      if (!BarcodeDetectorCtor) {
        stopBrowserScanner();
        setStatusError(true);
        setStatusMessage(zxingError instanceof Error ? zxingError.message : 'Compatible barcode scanner could not start.');
        return;
      }
    }
    ...
    try {
      ...
      stopBrowserScanner();
      setStatusError(true);
      setStatusMessage(error instanceof Error ? error.message : 'Camera access failed.');
    }
