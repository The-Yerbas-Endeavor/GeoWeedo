...
import ProductMeasurementTable from '@/components/ProductMeasurementTable';
...
export default async function ChemistryPage({ params }: Props) {
  ...
  const productRows = listings.map(listing => ({
    id: `${listing.productId}-${listing.batchId}`,
    productName: [listing.brandName, listing.productName].filter(Boolean).join(' — '),
    productHref: `/product-chemistry?product=${encodeURIComponent(listing.productId)}&batch=${encodeURIComponent(listing.batchId)}`,
    batchLabel: listing.batchNumber || listing.coaNumber || 'Verified batch',
    testedAt: listing.testedAt,
    value: listing.value,
    valueLabel: formatMeasurement(listing.value, listing.unit),
  }));
      ...
      <div className="weedoFactsPage">
        ...
        <article className="weedoFactsCard">
          <section>
            ...
            {productRows.length ? (
              <ProductMeasurementTable rows={productRows} />
            ) : (
