...
export default async function DispensaryProfilePage({params}:Props){
 ...
 const profile=getCommunityProfile(location.id),claimed=isClaimed(location.id),sponsorship=(await activeSponsorshipMap()).get(location.id),sponsored=Boolean(sponsorship),listed=location.kind==='dispensary'&&Boolean(location.active&&location.verified),profileTier=tier({sponsored,claimed,listed});
 return <main className={`dispensary-profile-page profile-tier-${profileTier.toLowerCase().replace(/[^a-z]+/g,'-')}`}><a className="profile-back" href="/">← Back to map</a><section className="dispensary-profile-shell"><div className="profile-kicker">GEOWEEDO DISPENSARY</div><div className="profile-status-row"><span className={`profile-tier-badge${sponsored?' sponsored':''}`}>{profileTier}</span>{claimed&&<span className="profile-owner-badge">✓ Owner verified</span>}{sponsored&&<span className="profile-sponsored-disclosure">Sponsored placement</span>}</div><h1>{location.name}</h1><p className="profile-location">{[location.streetAddress,location.city,location.region,location.postalCode,location.country].filter(Boolean).join(', ')}</p>{profile?.overview&&<p className="profile-owner-overview">{profile.overview}</p>}<div className="profile-primary-actions">{profile?.website||location.website?<a href={profile?.website||location.website} target="_blank" rel="noreferrer">Website</a>:null}{profile?.phone||location.phone?<a href={`tel:${profile?.phone||location.phone}`}>Call</a>:null}{claimed?<a href="/admin/my-dispensary">Owner profile editor</a>:<a href={`/owner?dispensary=${encodeURIComponent(location.id)}`}>Claim this dispensary</a>}</div>{sponsored&&<aside className="profile-sponsored-panel"><strong>Featured GeoWeedo profile</strong><p>This dispensary has active sponsored placement. Sponsorship changes presentation and placement, not reviews, licensing data, or organic search relevance.</p></aside>}<ModeratorDispensaryEditor locationId={location.id}/><DispensaryCommunityDetails locationId={location.id}/></section></main>;
}
