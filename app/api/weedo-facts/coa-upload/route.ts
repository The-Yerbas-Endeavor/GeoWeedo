...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!identifierValue || identifierValue.length > 2048) {
      return NextResponse.json({ error: 'A valid scanned identifier is required.' }, { status: 400 });
    }
    if (!(upload instanceof File)) {
      return NextResponse.json({ error: 'Choose an official SC Labs COA PDF.' }, { status: 400 });
    }
    if (upload.size < 5 || upload.size > MAX_PDF_BYTES) {
      return NextResponse.json({ error: 'COA PDF must be between 5 bytes and 15 MB.' }, { status: 413 });
    }
    ...
    return NextResponse.json({
      ...
      message: 'Official SC Labs COA parsed and attached as pending evidence.',
    });
  } catch (error) {
