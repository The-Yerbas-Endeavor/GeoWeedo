...
export async function GET(request: NextRequest) {
  ...
  const pendingRewardAtomic = Number(pendingRewards?.amount || 0);
  const totalAtomic = postedAtomic + pendingRewardAtomic;
  const availableAtomic = postedAtomic + heldDebitAtomic;
  return NextResponse.json({
    ...
    wallet: {
      ...
      postedAtomic,
      pendingRewardAtomic,
      heldAtomic: Math.abs(heldDebitAtomic),
      availableAtomic,
