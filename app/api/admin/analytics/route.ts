...
function individualVisitors(days:number,excludeAdmin:boolean,excludeIps:string[]){
 const database=getAnalyticsDb(),range=Math.max(1,Math.min(90,days)),since=new Date(Date.now()-range*86400000).toISOString(),ips=Array.from(new Set(excludeIps.filter(Boolean))),dailyHashes:string[]=[];
 for(let offset=0;offset<=range;offset++){const day=new Date(Date.now()-offset*86400000).toISOString().slice(0,10);for(const ip of ips)dailyHashes.push(analyticsNetworkHashForDay(ip,day));}
 const stableHashes=ips.map(analyticsStableNetworkHash),clauses=[`s.started_at>=?`],params:any[]=[since];
 if(excludeAdmin)clauses.push(`EXISTS (SELECT 1 FROM analytics_events ae WHERE ae.session_id=s.id AND ae.event_type='page_view' AND COALESCE(ae.path,'/') NOT LIKE '/admin%')`);
 if(dailyHashes.length){clauses.push(`(s.network_hash IS NULL OR s.network_hash NOT IN (${dailyHashes.map(()=>'?').join(',')}))`);params.push(...dailyHashes)}
 if(stableHashes.length){clauses.push(`(s.stable_network_hash IS NULL OR s.stable_network_hash NOT IN (${stableHashes.map(()=>'?').join(',')}))`);params.push(...stableHashes)}
 const where=clauses.join(' AND ');
 const rows=database.prepare(`SELECT s.visitor_id,MAX(COALESCE(s.user_id,'')) user_id,COUNT(*) sessions,MIN(s.started_at) first_seen,MAX(s.last_seen_at) last_seen,MAX(COALESCE(s.country,'')) country,MAX(COALESCE(s.region,'')) region,MAX(COALESCE(s.city,'')) city,MAX(COALESCE(s.screen_width,0)) screen_width,MAX(COALESCE(s.user_agent,'')) user_agent,MAX(COALESCE(s.landing_path,'')) landing_path,MAX(COALESCE(s.referrer,'')) referrer FROM analytics_sessions s WHERE ${where} GROUP BY s.visitor_id ORDER BY last_seen DESC LIMIT 500`).all(...params) as any[];
 const eventClauses=[`e.event_type='page_view'`,`e.created_at>=?`,`EXISTS (SELECT 1 FROM analytics_sessions s WHERE s.id=e.session_id AND ${where})`],eventParams:any[]=[since,...params];
 if(excludeAdmin)eventClauses.push(`COALESCE(e.path,'/') NOT LIKE '/admin%'`);
 const views=database.prepare(`SELECT e.visitor_id,COUNT(*) page_views FROM analytics_events e WHERE ${eventClauses.join(' AND ')} GROUP BY e.visitor_id`).all(...eventParams) as any[];
 const viewMap=new Map(views.map(row=>[String(row.visitor_id),Number(row.page_views||0)]));
 return rows.map(row=>({visitorId:String(row.visitor_id),userId:String(row.user_id||''),sessions:Number(row.sessions||0),pageViews:viewMap.get(String(row.visitor_id))||0,firstSeen:String(row.first_seen||''),lastSeen:String(row.last_seen||''),country:String(row.country||''),region:String(row.region||''),city:String(row.city||''),device:classifyDevice(Number(row.screen_width||0)),browser:classifyBrowser(String(row.user_agent||'')),landingPath:String(row.landing_path||''),referrer:String(row.referrer||'')}));
}
