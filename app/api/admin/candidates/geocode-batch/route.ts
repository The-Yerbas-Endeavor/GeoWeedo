...
async function oneLineLookup(item: DispensaryCandidate) {
  ...
  const response=await fetch(url,{headers:{Accept:'application/json','User-Agent':'GeoWeedo/0.7 (https://geoweedo.com)'},cache:'no-store'}); if(!response.ok)return null;
...
export async function POST(request:NextRequest){
  ...
  const all=await listCandidates(); const missingAll=all.filter(item=>item.status==='candidate'&&(!Number.isFinite(item.latitude)||!Number.isFinite(item.longitude))); const missing=requestedRegion?missingAll.filter(item=>String(item.region||'').trim().toLowerCase()===requestedRegion.toLowerCase()):missingAll; const candidates=missing.filter(item=>Boolean(item.streetAddress?.trim())).slice(0,limit); const candidateById=new Map(candidates.map(item=>[item.id,item])); const skippedWithoutStreet=missing.filter(item=>!item.streetAddress?.trim()).length;
  if(!candidates.length)return NextResponse.json({submitted:0,matched:0,batchMatched:0,fallbackMatched:0,unmatched:0,skippedWithoutStreet,remaining:missing.length,remainingAll:missingAll.length,region:requestedRegion||null});
  ...
  try{
    const response=await fetch('https://geocoding.geo.census.gov/geocoder/locations/addressbatch',{method:'POST',body:form,headers:{'User-Agent':'GeoWeedo/0.7 (https://geoweedo.com)'},cache:'no-store'}); if(!response.ok)throw new Error(`U.S. Census Geocoder returned ${response.status}`);
    ...
    const after=await listCandidates(); const afterMissingAll=after.filter(item=>item.status==='candidate'&&(!Number.isFinite(item.latitude)||!Number.isFinite(item.longitude))); const remaining=requestedRegion?afterMissingAll.filter(item=>String(item.region||'').trim().toLowerCase()===requestedRegion.toLowerCase()).length:afterMissingAll.length; const matched=batchMatched+fallbackMatched;
    return NextResponse.json({submitted:candidates.length,matched,batchMatched,fallbackMatched,fallbackAttempted:fallbackCandidates.length,unmatched:candidates.length-matched,skippedWithoutStreet,remaining,remainingAll:afterMissingAll.length,region:requestedRegion||null,provider:'U.S. Census Geocoder'},{headers:{'Cache-Control':'no-store'}});
