...
function parsedLocality(matchedAddress: string) { const parts=matchedAddress.split(',').map(p=>p.trim()).filter(Boolean); if(parts.length<3)return{}; const zipLike=/^\d{5}(?:-\d{4})?$/; let end=parts.length-1; const postalCode=zipLike.test(parts[end])?parts[end]:''; if(postalCode)end--; const region=parts[end]||''; const city=parts[end-1]||''; return {city,region,postalCode}; }
...
async function oneLineLookup(item: DispensaryCandidate) {
  ...
  return {latitude,longitude,matchedAddress,city:String(components.city||fallback.city||''),region:String(components.state||fallback.region||''),postalCode:String(components.zip||components.zipcode||fallback.postalCode||'')};
}
...
export async function POST(request:NextRequest){
  ...
  await runWithConcurrency(locationFallbackCandidates,1,async item=>{try{const match=await locationNameLookup(item);if(!match){await updateCandidate(item.id,{imageryStatus:'missing_coordinates',imageryMessage:`${item.region} name+city fallback did not find a high-confidence in-state OpenStreetMap match; manual review required.`});return;}await updateCandidate(item.id,{latitude:match.latitude,longitude:match.longitude,streetAddress:match.streetAddress||item.streetAddress,city:match.city||item.city,postalCode:match.postalCode||item.postalCode,imageryStatus:'unchecked',imageryCount:0,imageryCheckedAt:undefined,imageryMessage:`Coordinates matched by strict ${item.region} OpenStreetMap name+city fallback: ${match.matchedAddress}`});locationFallbackMatched++;}catch{}});
