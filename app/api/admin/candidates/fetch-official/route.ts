...
type SyncDetail={ok:boolean;source:string;fetched:number;added:number;existing:number;filled:number;fieldsFilled:number;geocoded:number;total?:number;error?:string};
...
async function syncSource(source:(typeof officialSources)[number]):Promise<SyncDetail>{try{const rows=await source.fetcher() as CandidateRow[];if(!rows.length)throw new Error(`${source.label} returned zero valid dispensary records.`);const result=await importCandidates(rows as any[]);return{ok:true,source:source.label,fetched:rows.length,added:result.added,existing:result.existing,filled:result.filled,fieldsFilled:result.fieldsFilled,geocoded:rows.filter(r=>Number.isFinite(r.latitude)&&Number.isFinite(r.longitude)).length,total:result.total};}catch(error){return{ok:false,source:source.label,fetched:0,added:0,existing:0,filled:0,fieldsFilled:0,geocoded:0,error:error instanceof Error?error.message:String(error)};}}
export async function POST(request:NextRequest){if(!getAdminFromRequest(request))return NextResponse.json({error:'Unauthorized.'},{status:401});const body=await request.json().catch(()=>null),preset=String(body?.preset||'');if(preset==='all'){const details:SyncDetail[]=[];for(const source of officialSources.filter(item=>item.preset!=='alaska-amco'&&item.preset!=='mississippi-mmcp'&&item.preset!=='new-mexico-ccd'&&item.preset!=='oklahoma-omma'&&item.preset!=='british-columbia-lcrb'))details.push(await syncSource(source));const successful=details.filter(d=>d.ok);return NextResponse.json({added:successful.reduce((s,d)=>s+d.added,0),existing:successful.reduce((s,d)=>s+d.existing,0),filled:successful.reduce((s,d)=>s+d.filled,0),fieldsFilled:successful.reduce((s,d)=>s+d.fieldsFilled,0),fetched:successful.reduce((s,d)=>s+d.fetched,0),geocoded:successful.reduce((s,d)=>s+d.geocoded,0),details,failed:details.length-successful.length,succeeded:successful.length,skipped:[{source:'Alaska AMCO',reason:'Temporarily unavailable for automated sync while Alaska transitions public licensing search to AK-ACCIS.'},{source:'Mississippi MMCP',reason:'Temporarily unavailable for automated sync because the official MMCP Business Search is browser-accessible but is timing out from the GeoWeedo production server.'},{source:'New Mexico CCD',reason:'Temporarily unavailable for automated sync because the current CCD public search does not expose a reliable machine-readable retailer roster.'},{source:'Oklahoma OMMA',reason:'Temporarily unavailable for automated bulk sync because OMMA publishes current dispensary totals and per-license verification, but not a public machine-readable bulk roster of active dispensary names and addresses.'}],source:'Official United States sync'},{status:successful.length?201:502});}const source=officialSources.find(item=>item.preset===preset);if(!source)return NextResponse.json({error:'Unknown official-source preset.'},{status:400});const detail=await syncSource(source);if(!detail.ok)return NextResponse.json({error:detail.error,details:[detail]},{status:502});return NextResponse.json({...detail,details:[detail]},{status:201});}