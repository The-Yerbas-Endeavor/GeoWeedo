...
export async function GET(request:NextRequest){if(!getAdminFromRequest(request))return NextResponse.json({error:'Unauthorized.'},{status:401});const db=getDatabase();
 ...
 const walletUsers=db.prepare(`SELECT w.id AS walletId,w.user_id AS userId,u.username,u.display_name AS displayName,u.yerbas_address AS yerbasAddress,(SELECT a.address FROM wallet_addresses a WHERE a.wallet_id=w.id AND a.address_type='deposit' AND a.active=1 ORDER BY a.created_at DESC LIMIT 1) AS activeDepositAddress FROM wallets w JOIN users u ON u.id=w.user_id ORDER BY COALESCE(u.display_name,u.username,u.yerbas_address,u.id) COLLATE NOCASE`).all();
 const depositAddresses=db.prepare(`SELECT a.id,a.address,a.address_type,a.label,a.active,a.created_at,w.user_id,u.username,u.display_name FROM wallet_addresses a JOIN wallets w ON w.id=a.wallet_id LEFT JOIN users u ON u.id=w.user_id ORDER BY a.active DESC,a.created_at DESC LIMIT 250`).all();
 ...
 return NextResponse.json({rpc,summary:{wallets:Number(counts.wallets||0),activeAddresses:Number(counts.activeAddresses||0),pendingDeposits:Number(counts.pendingDeposits||0),pendingWithdrawals:Number(counts.pendingWithdrawals||0),pendingRewards:Number(counts.pendingRewards||0),ledgerBalanceYerb:y(totals.ledgerBalance),confirmedDepositsYerb:y(totals.deposits),sentWithdrawalsYerb:y(totals.withdrawals),postedRewardsYerb:y(totals.rewards)},walletUsers,depositAddresses,recentDeposits:recentDeposits.map((r:any)=>({...r,amountYerb:y(r.amount_atomic)})),recentWithdrawals:recentWithdrawals.map((r:any)=>({...r,amountYerb:y(r.amount_atomic),feeYerb:y(r.fee_atomic)})),recentLedger:recentLedger.map((r:any)=>({...r,amountYerb:y(r.amount_atomic)}))},{headers:{'Cache-Control':'no-store'}});}

export async function POST(request:NextRequest){
 const admin=getAdminFromRequest(request);if(!admin)return NextResponse.json({error:'Unauthorized.'},{status:401});
 const body=await request.json().catch(()=>null);const walletId=String(body?.walletId||'').trim();
 if(!walletId)return NextResponse.json({error:'walletId is required.'},{status:400});
 const db=getDatabase();
 const wallet=db.prepare(`SELECT w.id,w.user_id,u.username,u.display_name FROM wallets w JOIN users u ON u.id=w.user_id WHERE w.id=?`).get(walletId) as any;
 if(!wallet)return NextResponse.json({error:'Wallet not found.'},{status:404});
 const existing=db.prepare(`SELECT id,address FROM wallet_addresses WHERE wallet_id=? AND address_type='deposit' AND active=1 ORDER BY created_at DESC LIMIT 1`).get(walletId) as any;
 if(existing?.address)return NextResponse.json({address:existing.address,existing:true,walletId,userId:wallet.user_id});
 try{
  let address:string;
  try{address=String(await yerbasRpc<string>('getnewaddress',[`geoweedo:${walletId}`]));}
  catch{address=String(await yerbasRpc<string>('getnewaddress',[]));}
  if(!address)throw new Error('Yerbas Core returned an empty address.');
  const now=new Date().toISOString();
  db.prepare(`INSERT INTO wallet_addresses (id,wallet_id,address,address_type,label,active,created_at) VALUES (?,?,?,'deposit',?,1,?)`).run(`wa-${crypto.randomUUID()}`,walletId,address,`GeoWeedo ${wallet.display_name||wallet.username||wallet.user_id}`,now);
  db.prepare(`INSERT INTO audit_log (id,actor_type,actor_id,action,entity_type,entity_id,metadata_json,created_at) VALUES (?,?,?,?,?,?,?,?)`).run(`audit-${crypto.randomUUID()}`,'admin',admin.id,'wallet.deposit_address_created','wallet',walletId,JSON.stringify({userId:wallet.user_id,address}),now);
  return NextResponse.json({address,existing:false,walletId,userId:wallet.user_id},{status:201});
 }catch(error){return NextResponse.json({error:error instanceof Error?error.message:'Could not generate deposit address.'},{status:502});}
}
