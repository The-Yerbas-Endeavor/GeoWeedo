...
export async function POST(request: NextRequest) {
  ...
  db.prepare(`INSERT INTO wallet_ledger (id, wallet_id, entry_type, amount_atomic, status, reference_type, reference_id, memo, metadata_json, created_at)
    ...
    .run(id, player.wallet_id, Math.round(amount * ATOMIC), reference, String(body?.reason || 'GeoWeedo reward'), JSON.stringify({ adminId: admin.id }), now);
  const reward = mapReward({ id, user_id: playerId, amount_atomic: Math.round(amount * ATOMIC), memo: String(body?.reason || 'GeoWeedo reward'), reference_id: reference, status: 'pending', txid: null, created_at: now, posted_at: null });
