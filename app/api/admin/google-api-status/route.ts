...
import {
  ...
  getGooglePanoramaUsageForCurrentMonth,
...
function estimateCost(events: number) {
  ...
  return {
    ...
    estimatedUsd: (billable / 1000) * DYNAMIC_STREET_VIEW_USD_PER_1000,
  };
}
...
export async function GET(request: NextRequest) {
  ...
  const days = Math.min(
    90,
    Math.max(1, Number.isFinite(requestedDays) ? Math.floor(requestedDays) : 7),
  );
  ...
  const warningLevel =
    allowancePct >= 100 ? 100 :
    allowancePct >= 90 ? 90 :
    allowancePct >= 75 ? 75 :
    allowancePct >= 50 ? 50 : 0;
  ...
  return NextResponse.json({
    ...
    placesKeySource: placesDedicatedKeyConfigured
      ? 'dedicated'
      : mapsKeyConfigured
        ? 'maps-fallback'
        : 'missing',
    ...
    cost: {
      ...
      currency: 'USD',
      pricingModel: 'Google Maps Platform pay-as-you-go estimate',
      freeMonthlyEvents: DYNAMIC_STREET_VIEW_FREE_MONTHLY,
      usdPer1000: DYNAMIC_STREET_VIEW_USD_PER_1000,
      monthEvents: month.total,
      monthRows: month.rows,
      allowancePct,
      remainingFreeEvents: Math.max(0, DYNAMIC_STREET_VIEW_FREE_MONTHLY - month.total),
      billableEvents: currentEstimate.billableEvents,
      estimatedMonthCostUsd: currentEstimate.estimatedUsd,
      projectedMonthEvents: projectedEvents,
      projectedMonthCostUsd: projectedEstimate.estimatedUsd,
      warningLevel,
      warningThresholds: [50, 75, 90, 100],
      billingMonthUtc: `${now.getUTCFullYear()}-${String(now.getUTCMonth() + 1).padStart(2, '0')}`,
      isEstimate: true,
    },
    accounting: {
      source: 'GeoWeedo server request counters',
      includesGoogleBilling: false,
      note: 'ESTIMATE ONLY. Panorama counts are recorded by GeoWeedo when it instantiates a Google Dynamic Street View panorama. Estimated cost applies Google pay-as-you-go list pricing to those observed events. Google Cloud billing, account credits, negotiated pricing, subscription plans, quota-side adjustments, failed/duplicate billing classification, and traffic outside GeoWeedo are not included.',
    },
  });
}
