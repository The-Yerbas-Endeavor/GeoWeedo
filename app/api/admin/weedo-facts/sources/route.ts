...
import { beginSourceUpdate, getSourceSummaries, sourceCanStart, type WeedoDataSourceId } from '@/lib/weedoDataSources';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    child.unref();
    return NextResponse.json({ ok: true, sourceId, state: 'running' }, { status: 202 });
  } catch (error) {
