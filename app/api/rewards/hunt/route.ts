...
export async function POST(request: NextRequest) {
  ...
  return NextResponse.json({ gameId, totalScore: score, amountYerb, status: amountAtomic <= 0 ? 'daily_cap_reached' : mapStatus(ledgerStatus), dailyCapYerb: policy.dailyCapYerb, dailyRemainingYerb: Number(Math.max(0, remainingDaily - amountYerb).toFixed(8)), reviewRequired: policy.reviewRequired, rewardedGamesToday: timing.rewardedGamesToday + (amountAtomic > 0 ? 1 : 0) }, { status: 201 });
}
