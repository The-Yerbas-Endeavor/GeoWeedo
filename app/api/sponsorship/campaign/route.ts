...
import {activeGameCampaign,getGameCampaign,recordGameCampaignEvent,type GameCampaignType} from '@/lib/gameSponsorship';
...
export async function GET(request:NextRequest){
 ...
 return NextResponse.json({game,campaign:campaign?publicCampaign(campaign):null});
}

export async function POST(request:NextRequest){
 const body=await request.json().catch(()=>({}));
 const campaignId=String(body.campaignId||'').trim(),eventType=String(body.eventType||'').trim();
 if(!campaignId||!['game_impression','game_completed'].includes(eventType))return NextResponse.json({error:'Invalid campaign event.'},{status:400});
 const campaign=getGameCampaign(campaignId);
 if(!campaign)return NextResponse.json({ok:true,recorded:false});
 const metadata=body.metadata&&typeof body.metadata==='object'&&!Array.isArray(body.metadata)?body.metadata:{};
 const recorded=recordGameCampaignEvent(campaignId,eventType as 'game_impression'|'game_completed',metadata);
